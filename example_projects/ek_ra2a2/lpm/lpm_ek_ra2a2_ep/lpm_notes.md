...
* 1 x Micro USB cable or 1 x Type C USB cable (For MCK-RA4T1 , MCK-RA6T2 , MCK-RA6T3, MCK-RA8T1) or 1 x Mini USB cable (For RSSK-RA6T1)
...
* 1 x Pmod BTN (Only MCK-RA4T1, MCK-RA6T3, RSSK-RA6T1, MCK-RA8T1 have to use)
...
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
|-----------------------------------------|-------------------|----------------|------------|
| configuration.xml > g_lpm_sw_standby Low Power Modes (r_lpm) > Settings > Property > Module g_lpm_deep_sleep Low Power Modes (r_lpm) > Deep Sleep and Standby Options > Wake Sources > AGTW1 Underflow | ☐ | ☑ | Select AGTW1 Underflow Interrupt as the source to cancel SW Standby mode. |
...
## Verifying operation ##
1. Download the example project to the RA board using the USB Debug port. J-Link may be disconnected when MCU enter LPM mode. Recommend to stop debugging after downloading EP.
...
5. Press the user push-button Sn or wait 10 seconds to enter and cancel LPM mode. It will turn off LED1 before entering each LPM mode and turn on LED1 after exiting each LPM mode.
...
* The serial log when use push-button Sn to cancel/end lpm mode 
