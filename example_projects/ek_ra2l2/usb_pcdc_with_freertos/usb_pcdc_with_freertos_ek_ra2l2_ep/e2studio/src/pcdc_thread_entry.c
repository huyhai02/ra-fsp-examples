...
/* Buffer to store the user data */
uint8_t g_user_data_buf[DATA_LEN] __attribute__((aligned(2))) = {RESET_VALUE};
...
const usb_descriptor_t g_usb_pcdc_descriptor =
{
 ...
 NUM_STRING_DESCRIPTOR           /* Number of entry String Descriptor */
};
...
/*******************************************************************************************************************//**
 *  @brief      Initialize UART configuration values based on user input values through serial terminal.
 *  @param[in]  p_uart_test_cfg   Pointer to store UART configuration properties.
                p_line_coding     Update the line coding member values.
 *  @retval     None.
 **********************************************************************************************************************/
 void set_uart_line_coding_cfg(uart_cfg_t *p_uart_test_cfg, const volatile usb_pcdc_linecoding_t * p_line_coding)
{
...
void pcdc_thread_entry(void *pvParameters)
{
    ...
    /* Open the UART with initial configuration */
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("\r\n** R_"SCI_TYPE"_UART_Open API failed **\r\n");
            APP_ERR_TRAP(err);
        }
    ...
    while (1)
    {
        ...
        if (PERIPHERAL_UART == q_instance.peripheral)
        {
            /* Start sending the data */
            {
                ...
                {
                    /* Pull out as many items from queue as possible */
                    ...
                    if (pdTRUE == err_semaphore)
                    {
                        ...
                        err = R_USB_Write(&g_basic_ctrl, &g_usb_tx_buffer[0], (uint32_t) unload_count * rx_data_size,\
                                          USB_CLASS_PCDC);
                        if (FSP_SUCCESS != err)
                        {
        ...
        if (PERIPHERAL_USB == q_instance.peripheral)
        {
            ...
            if ((true == b_usb_attach))
            {
                if (0U < q_instance.u.data_size)
                {
                    ...
                    if (FSP_SUCCESS != err)
                    {
                        APP_ERR_PRINT("\r\n** R_"SCI_TYPE"_UART_Write API failed **\r\n");
                        deinit_usb();
                        deinit_uart();
                        APP_ERR_TRAP(err);
                    }
...
/*******************************************************************************************************************//**
 *  @brief      De-initialize initialized UART module.
 *  @param[in]  None.
 *  @retval     None.
 **********************************************************************************************************************/
void deinit_uart(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\n** R_"SCI_TYPE"_UART_Close API failed **\r\n");
    }
}

/*******************************************************************************************************************//**
 *  @brief      De-initialize initialized USB instance.
 *  @param[in]  None.
 *  @retval     None.
 **********************************************************************************************************************/
...
/*******************************************************************************************************************//**
 * @brief       USB PCDC callback function for FreeRTOS. Handles USB CDC events such as read/write completion,
 *              configuration, class requests, attach/detach, and suspend/resume states.
 * @param[in]   p_pcdc_event  Pointer to USB event information.
 * @param[in]   task          Handle to the calling task (unused).
 * @param[in]   state         USB on/off state (unused).
 * @retval      None.
 **********************************************************************************************************************/
...
#if BSP_PERIPHERAL_SCI_B_PRESENT
                        err = R_SCI_B_UART_BaudCalculate(g_baud_rate, g_enable_bitrate_modulation,
                                                         g_error_rate_x_1000, &g_baud_setting);
#else
                        err = R_SCI_UART_BaudCalculate(g_baud_rate, g_enable_bitrate_modulation,
                                                       g_error_rate_x_1000, &g_baud_setting);
#endif /* BSP_PERIPHERAL_SCI_B_PRESENT */
                        if (FSP_SUCCESS != err)
                        {
                            APP_ERR_PRINT("\r\n** R_"SCI_TYPE"_UART_BaudCalculate API failed **\r\n");
                            deinit_usb();
                            deinit_uart();
                            APP_ERR_TRAP(err);
                        }
            ...
            else if (USB_PCDC_SET_CONTROL_LINE_STATE == (p_pcdc_event->setup.request_type & USB_BREQUEST))
            {
                ...
                if (FSP_SUCCESS == err)
                {
                    if (FSP_SUCCESS == err)
                    {
                        ...
                        /* Toggle the line state if the flow control pin is set to a value
                         * (other than SCI_UART_INVALID_16BIT_PARAM) */
                        ...
                        APP_PRINT("\r\n******Line State*******\r\nbdtr = %d\r\nbrts = %d\r\n"\
                                  "***********************\r\n", g_control_line_state.bdtr, g_control_line_state.brts);
                    }
                }
...
/*******************************************************************************************************************//**
 * @brief       UART user callback. Handles UART events such as receive, transmit complete, and errors.
