...
uint8_t g_ftp_client0_stack_memory[G_FTP_CLIENT0_WINDOW_SIZE] BSP_PLACE_IN_SECTION(".stack.g_ftp_client0")\
                                                              BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
void ftp_client_thread_entry(void)
{
    ...
    memset(&server_ip_address, RESET_VALUE, sizeof(server_ip_address));
...
static void ftp_client_init(void)
{
    ...
    status = nx_ftp_client_create(&g_ftp_client0, "g_ftp_client0", &g_ip0, G_FTP_CLIENT0_WINDOW_SIZE, &g_packet_pool0);
    /* Check status */
    if (NX_SUCCESS != status)
    {
...
static UINT create_ftp_file(ULONG file_size)
{
    ...
    /* Fill the file buffer to the provided file size */
