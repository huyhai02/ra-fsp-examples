...
|configuration.xml > HTTP Client Thread > HTTP Client on DA16XXX (rm_http_onchip_da16xxx) > Wifi DA16XXX Framework Driver (rm_wifi_da16xxx) > DA16XXX transport on UART (rm_at_transport_da16xxx_uart) > Properties > Settings > Property > Module Reset Port | 06 | 07 |Setup reset port for the MCU.|
|configuration.xml > HTTP Client Thread > HTTP Client on DA16XXX (rm_http_onchip_da16xxx) > Wifi DA16XXX Framework Driver (rm_wifi_da16xxx) > DA16XXX transport on UART (rm_at_transport_da16xxx_uart) > Properties > Settings > Property > Module Reset Pin | 03 | 08 |Setup reset pin for the MCU.|
|configuration.xml > g_uart0 UART (r_sci_uart) > module > general > channel | 0 | 0 | PMOD2 is available on UART channel 0. |
|configuration.xml > g_uart0 UART (r_sci_uart) > Pins > TXD0 | P411 | P411 | Select TX pin. |
|configuration.xml > g_uart0 UART (r_sci_uart) > Pins > RXD0 | P410 | P410 | Select RX pin. |
