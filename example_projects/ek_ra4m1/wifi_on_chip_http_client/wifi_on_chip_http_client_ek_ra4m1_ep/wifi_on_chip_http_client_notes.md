...
|configuration.xml > HTTP Client Thread > HTTP Client on DA16XXX (rm_http_onchip_da16xxx) > Wifi DA16XXX Framework Driver (rm_wifi_da16xxx) > DA16XXX transport on UART (rm_at_transport_da16xxx_uart) > Properties > Settings > Property > Module Reset Pin | 03 | 07 |Setup reset pin for the MCU.|
|configuration.xml > g_uart0 UART (r_sci_uart) > module > general > channel | 0 | 0 | PMOD A is available on UART channel 0. |
|configuration.xml > g_uart0 UART (r_sci_uart) > Pins > TXD_MOSI0 | P101 | P101 | Select TX pin. |
|configuration.xml > g_uart0 UART (r_sci_uart) > Pins > RXD_MISO0 | P100 | P100 | Select RX pin. |
