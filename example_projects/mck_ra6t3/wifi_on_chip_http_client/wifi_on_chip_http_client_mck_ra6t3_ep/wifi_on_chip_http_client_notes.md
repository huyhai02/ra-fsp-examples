...
|configuration.xml > g_uart0 UART (r_sci_uart) > module > general > channel | 0 | 9 | PMOD Type 3A is available on UART channel 9. |
|configuration.xml > g_uart0 UART (r_sci_uart) > Pins > TXD9 | P411 | P109 | Select TX pin. |
|configuration.xml > g_uart0 UART (r_sci_uart) > Pins > RXD9 | P410 | P110 | Select RX pin. |
