...
#define ADC_EXAMPLE_CAL_VCC_VOLT                         (3.0)      /* 3.0 Volt reference */
#define ADC_EXAMPLE_TEMPERATURE_RESOLUTION               (12U)      /* Set ADC to 12-bit resolution */
#define ADC_EXAMPLE_REFERENCE_CALIBRATION_TEMPERATURE    (140.0)    /* Reference temperature at calibration */
...
static fsp_err_t read_die_temperature(float * celcius_temperature)
{
    ...
    cal140 = (float32_t) p_adc_info.calibration_data; /* Cast uint32_t calibration data to float for calculation */
    ...
    v1 = (float32_t) ((ADC_EXAMPLE_CAL_VCC_VOLT * cal140) / pow(2, ADC_EXAMPLE_TEMPERATURE_RESOLUTION));
 ...
 * Return Value : None
 *********************************************************************************************************************/
static fsp_err_t test_temperature_change(void)
{
    fsp_err_t fsp_err = FSP_SUCCESS;

    float32_t mcu_temp_f = 86.0;
    float32_t mcu_temp_c = 30.0;
    float32_t tmp;
    uint16_t  d;
    uint16_t  r;
    ...
    else
    {
        ...
        fsp_err = read_die_temperature(&mcu_temp_c);
...
void button_irq7_callback(external_irq_callback_args_t *p_args)
{
    ...
    if ((uxBits & (STATUS_LPM_DISABLE)) != (STATUS_LPM_DISABLE))
    {
...
/* SW 1 */
/**********************************************************************************************************************
 * Function Name: button_irq6_callback
 * Description  : SW1 interrupt handler.
 * Argument     : p_args
 * Return Value : None
 *********************************************************************************************************************/
void button_irq6_callback(external_irq_callback_args_t *p_args)
{
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    BaseType_t xResult                  = pdFAIL;
    ...
    if ((uxBits & (STATUS_LPM_SWITCH)) != (STATUS_LPM_SWITCH))
    {
        /* Check source flag state */
...
/**********************************************************************************************************************
 End of function button_irq6_callback
 *********************************************************************************************************************/
