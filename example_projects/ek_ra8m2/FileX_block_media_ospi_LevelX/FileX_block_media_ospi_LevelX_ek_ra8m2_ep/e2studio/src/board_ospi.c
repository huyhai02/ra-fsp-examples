...
static fsp_err_t ospi_b_write_enable(void)
{
    ...
    {
        RETURN_ERR_STR(FSP_ERR_ABORTED, "Write enable FAILED\r\n");
    }
    return err;
}
/***********************************************************************************************************************
 ...
 *  Arguments[in]: timeout         Maximum waiting time.
 *  Return Value : FSP_SUCCESS     Upon successful operation.
...
fsp_err_t ospi_b_wait_operation(uint32_t timeout)
{
    ...
    while (status.write_in_progress)
    {
        ...
        {
            RETURN_ERR_STR(FSP_ERR_TIMEOUT, "OSPI time out occurred\r\n");
        }
        R_BSP_SoftwareDelay(1, OSPI_B_TIME_UNIT);
        timeout --;

        PRINT_INFO_STR(RESET_VALUE == (timeout % PERIODIC_PRINT_OUT) ? ".":"");
        PRINT_INFO_STR(RESET_VALUE == (timeout % PERIODIC_LINE_FEED) ? "\r\n":"");
    }
    return err;
}
/***********************************************************************************************************************
...
fsp_err_t ospi_b_init(void)
{
    ...
    /* Set active OSPI instance for initialize */
    ...
    RETURN_ERR_STR(err, "ospi_b_write_enable FAILED\r\n");
    ...
    {
        RETURN_ERR_STR(FSP_ERR_ABORTED, "Verify CFR2V register data FAILED\r\n");
    }
    ...
    {
        RETURN_ERR_STR(FSP_ERR_ABORTED, "Verify CFR3V register data FAILED\r\n");
    }
    ...
    if (OSPI_B_DATA_CFR4V_REGISTER != (uint8_t)transfer.data)
    {
        ...
        {
            RETURN_ERR_STR(FSP_ERR_ABORTED, "Verify CFR4V register default data FAILED\r\n");
        }
        PRINT_INFO_STR("The CFR4V register still in default data\r\n");
        ...
        if (OSPI_B_DATA_CFR4V_REGISTER != (uint8_t)transfer.data)
        {
            RETURN_ERR_STR(FSP_ERR_ABORTED, "Verify CFR4V register data FAILED\r\n");
        }
    ...
    /* Set active OSPI instance for FileX */
 ...
 *                 Any other error code apart from FSP_SUCCESS Unsuccessful operation.
 **********************************************************************************************************************/
fsp_err_t erase_ospi_flash(void)
{
    fsp_err_t err = FSP_SUCCESS;
    ...
    /* Re-setup calibrate data after erase the entire OSPI Flash */
    err = ospi_b_setup_calibrate_data();
    RETURN_ERR_STR(err, "ospi_b_setup_calibrate_data FAILED\r\n");
    ...
    PRINT_INFO_STR("\r\nErased entire Flash successfully\r\n");
    PRINT_INFO_STR("\r\nPlease format media after erase the OSPI flash");
...
static fsp_err_t ospi_b_setup_calibrate_data(void)
{
...
#elif defined (BOARD_RA8M2_EK)
    uint32_t g_auto_calibration_data[4];
    uint32_t g_auto_calibration_data_spi[] =
    {
        0xFFFF0000U,
        0x0800FF00U,
        0xFF0000F7U,
    ...
    uint32_t g_auto_calibration_data_opi[] =
    {
        0xFFFF0000U,
        0x000800FFU,
        0x00FFF700U,
    ...
    {
        memcpy(g_auto_calibration_data, g_auto_calibration_data_spi, sizeof(g_auto_calibration_data));
    }
    else
    {
        memcpy(g_auto_calibration_data, g_auto_calibration_data_opi, sizeof(g_auto_calibration_data));
    }
#endif

    /* Verify auto-calibration data */
    if (RESET_VALUE != memcmp((uint8_t *)(OSPI_B_AUTO_CALIB_ADDRESS), &g_auto_calibration_data,
                              sizeof(g_auto_calibration_data)))
    {
        /* Erase the flash sector that stores auto-calibration data */
#if defined (BOARD_RA8D1_EK)
        err = R_OSPI_B_Erase(g_ospi_b_ctrl, (uint8_t *)OSPI_B_AUTO_CALIB_ADDRESS,
                             OSPI_B_SECTOR_256K_SIZE);
#elif defined (BOARD_RA8M2_EK)
        err = R_OSPI_B_Erase(g_ospi_b_ctrl, (uint8_t *)OSPI_B_AUTO_CALIB_ADDRESS,
                             OSPI_B_SECTOR_4K_SIZE);
#endif
        RETURN_ERR_STR(err, "R_OSPI_B_Erase API FAILED\r\n");
...
static fsp_err_t ospi_b_set_protocol_to_opi(void)
{
    ...
    else if (SPI_FLASH_PROTOCOL_EXTENDED_SPI == g_ospi_b_ctrl->spi_protocol)
    {
        ...
        {
            RETURN_ERR_STR(FSP_ERR_ABORTED, "Verify CFR5V register data FAILED\r\n");
        }
#elif defined (BOARD_RA8M2_EK)
        /* Write to CR2 Register to Configure flash device interface mode */
        ...
        {
            RETURN_ERR_STR(FSP_ERR_ABORTED, "R_OSPI_B_DirectTransfer API FAILED!!!\r\n");
        }
#endif
    }
    else
    {
        RETURN_ERR_STR(FSP_ERR_INVALID_MODE, "Flash device do not support this mode\r\n");
    }
