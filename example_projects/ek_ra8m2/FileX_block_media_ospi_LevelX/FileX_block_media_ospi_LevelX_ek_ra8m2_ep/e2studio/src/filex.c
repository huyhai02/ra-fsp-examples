...
UINT file_system_init(void)
{
    ...
    time.month = get_month((const char *)(__DATE__) + MONTH_INDEX);
    time.date = atoi((const char *)(__DATE__) + DATE_INDEX);
    time.year = atoi((const char *)(__DATE__) + YEAR_INDEX);

    /* Extract current Time from compiler MACROS */
    time.hour = atoi((const char *)(__TIME__) + HOUR_INDEX);
    time.min  = atoi((const char *)(__TIME__) + MIN_INDEX);
    time.sec  = atoi((const char *)(__TIME__) + SEC_INDEX);
...
static UINT media_format(void)
{
	...
	for (uint32_t i = RESET_VALUE; i <= g_rm_levelx_nor_spi_cfg.size ; i+= OSPI_B_SECTOR_4K_SIZE)
	{
	    status = (UINT) R_OSPI_B_Erase(g_ospi_b_ctrl, (uint8_t *)FILEX_START_ADDRESS + i, OSPI_B_SECTOR_4K_SIZE);
#endif
...
#elif defined (BOARD_RA8M2_EK)
		status = (UINT) ospi_b_wait_operation(OSPI_B_TIME_ERASE_4K);
#endif
