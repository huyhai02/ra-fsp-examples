...
Note: 
  ...
  - RA boards supported for J-Link OB VCOM: EK-RA8D1, EK-RA8M1, MCK-RA8T1 and EK-RA8D2.
...
### Hardware ###
Supported RA boards: EK-RA4M2, EK-RA4M3, EK-RA6M3, EK-RA6M4, EK-RA6M5, EK-RA8D1, EK-RA8M1, MCK-RA8T1, EK-RA8D2
...
* For EK-RA8D2:
	* High-Speed (Default):  
      * Connect the USB device to the J7 connector using a Type-C USB host cable.
      * Connect the Type-C USB end of the Type-C USB device cable to Type-C USB Debug port (J10) of the RA board and Connect the other end of this cable to the USB port of the host PC.
...
| configuration.xml > Stacks > Threads > USB HCDC Thread > Properties > Settings > Property > Thread > Priority | 1 | 2 | This is changed to set thread priority. |
| configuration.xml > Stacks > Threads > USB HCDC Thread > g_hcdc0 USB HCDC > Properties > Settings > Property > Common > Bulk Input Transfer Pipe | USB PIPE4 | USB PIPE3 | This is changed to set the USB pipe used. |
| configuration.xml > Stacks > Threads > USB HCDC Thread > g_hcdc0 USB HCDC > Properties > Settings > Property > Common > Bulk Output Transfer Pipe | USB PIPE5 | USB PIPE4 | This is changed to set the USB pipe used. |
...
| API Name    | Usage                                                                          |
|-------------|--------------------------------------------------------------------------------|
| RM_FREERTOS_PLUS_FAT_Open | This API is used to initialize lower layer media device. |
| RM_FREERTOS_PLUS_FAT_MediaInit | This API is used to initialize the media device. |
| RM_FREERTOS_PLUS_FAT_DiskInit | This API is used to initialize a FreeRTOS+FAT disk structure. |
| RM_FREERTOS_PLUS_FAT_DiskDeinit | This API is used to deinitialize a FreeRTOS+FAT disk structure. |
| RM_FREERTOS_PLUS_FAT_Close | This API is used to close media device. |
| ff_fopen | This API is used to open a file. |
| ff_fclose | This API is used to flush then close a file. |
| ff_fread | This API is used to read data from a file. |
| ff_fwrite | This API is used to write data to a file. |
| ff_remove | This API is used to remove (delete, or unlink) a file. |
| FF_Format | This API is used to format a partition of a disk. |
| FF_Mount | This API is used to mount the specified partition. |
| FF_FS_Add | This API is used to add a mounted partition to the FreeRTOS-Plus-FAT virtual file system. |
| stdioGET_ERRNO | This API is used to get the error code. |
| R_USB_HostControlTransfer |	This API is used to perform settings and transmission processing when transmitting a setup packet. |
| R_USB_Write | This API is used to request USB data write. |
| R_USB_Read | This API is used to request USB data read. |
| R_SCI_B_UART_Open | This API is used to initialize the UART driver. |
| R_SCI_B_UART_Write | This API is used to write data to the UART interface. |
| R_SCI_B_UART_Read | This API is used to read data to the UART interface. |
| R_SCI_B_UART_Abort | This API is used to abort the read process when the USB device is disconnected. |
| R_SCI_B_UART_Close | This API is used to deinitialize the UART driver. |
