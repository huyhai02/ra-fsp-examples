...
void rtt_thread_entry(void)
{
    ...
    while (1)
    {
        ...
        if (APP_CHECK_DATA)
        {
            ...
            if (TX_SUCCESS != err)
            {
                APP_PRINT("memory_allocate_rtt has failed, consider increasing byte pool memory and confirm "\
...
/*******************************************************************************************************************//**
 * @brief     This function receives messages from the user thread through tx_queue_receive
 *            and displays messages to RTT viewer. Also released print message allocated block.
 * @param[IN] None.
 * @retval    None.
 **********************************************************************************************************************/
static void process_rtt_op_msg(VOID)
...
/*******************************************************************************************************************//**
 * @brief     This function is called by the User/Application Thread.
 *            This is called by each user thread wherever the user wishes to
...
static UINT memory_allocate_rtt(TX_BYTE_POOL *pool, rtt_msg_t **p_buf, uint32_t size)
{
    ...
    /* Block allocate for the user input */
...
/*******************************************************************************************************************//**
 * @brief      User input read function. To be called by the user thread for receiving RTT input.
 *             message once received through queue (be it RTT sender or receiving queue) is being released.
 *             Note: sender queue for RTT thread is the user input from RTT Viewer.
 ...
 *             Mandatory to call in the user thread.
 * @param[IN]  None.
 * @retval     None.
 **********************************************************************************************************************/
