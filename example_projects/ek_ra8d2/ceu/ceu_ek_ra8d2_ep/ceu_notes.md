...
### Hardware ###
* Supported RA Boards: EK-RA8D1, FPB-RA8E1, EK-RA8D2.
...
Connect the USB Debug port on the RA board to the host PC via a Type-C USB cable.
  ...
  * For EK-RA8D2:
    ...
    * Detail connection: Connect the camera module to the Camera Expansion Port J35 on the back side of the EK-RA8D2 board using the Camera FFC cable.  
    ![Camera_connection](images/Connection_RA8D2_with_OV5640.jpg "Camera Connection")  
...
* The images below show the ceu EP operation:

  * **EP Information:**

    ![rtt_viewer_ep_information](images/rtt_viewer_information.jpg "RTT Viewer EP Information")

  * **Pattern test mode (Default):**  
    Define `CAM_TEST_PATTERN (0U)` macro in **camera_sensor.h**.
    ...
    * Camera Test pattern image stored in SRAM and SDRAM:

      ![Camera_test_pattern_color_bars](images/cam_test_pattern_color_bars.jpg "Test Pattern when displayed in SRAM")

  * **Actual image test mode:**  
    Define `CAM_TEST_PATTERN (1U)` macro in **camera_sensor.h**.

    * RTT Viewer operation of capturing an image using camera sensor:
    ...
    * The images below show image captured on Raw Image rendering:
      - **SXGA image stored in SDRAM:**

        ![raw_image_sxga_sdram](images/image_sxga_sdram.jpg "Raw Image SXGA in SDRAM")

      - **VGA image stored in SRAM:**
...
* CAM_TEST_PATTERN in camera_sensor.h is used to enable/disable using test pattern of camera sensor. It is enabled by default and a matching result between captured data and test pattern will be shown in RTT Viewer for this case. Disable it to capture live data from the camera.
