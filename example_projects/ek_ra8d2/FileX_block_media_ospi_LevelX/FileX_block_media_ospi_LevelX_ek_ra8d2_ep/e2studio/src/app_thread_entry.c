...
void app_thread_entry(void)
{
    ...
    status = tx_event_flags_get(&g_request_event, REQUEST_START_APP_THREAD, TX_OR_CLEAR, &actual_event,
                                TX_WAIT_FOREVER);
    ...
    while (true)
    {
        ...
        actual_event = RESET_VALUE;
        status = tx_event_flags_get(&g_request_event, FILE_SYSTEM_REQUEST_MASK, TX_OR_CLEAR, &actual_event,
                                    TX_WAIT_FOREVER);
        if (TX_SUCCESS != status)
        {
        ...
        if (FX_SUCCESS != status)
        {
            PRINT_ERR_STR("filex_ospi_levelx_operation failed\r\n");
            PRINT_ERROR_TRAP(status);
        }
