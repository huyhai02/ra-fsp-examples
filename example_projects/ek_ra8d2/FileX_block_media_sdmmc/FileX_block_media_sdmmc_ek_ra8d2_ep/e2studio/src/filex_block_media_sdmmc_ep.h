...
#else
#define CARD_TYPE     "SD"
#endif /* BOARD_RA8T1_MCK) || defined (BOARD_RA8T2_MCK */
...
#define EP_INFO                 "\r\nThe example project shows the operation of a FileX interface on block"\
								...
								"\r\nperform various file operations (open, close, read, write, format), and"\
								"\r\noptionally adding time stamp to the file, while creating the file on the"\
								"\r\nthe SD card (except for MCK-RA8T1, MCK-RA8T2 which use MicroSD card)."\
								"\r\nIn summary, it showcases the FileX functionality using Block Media SDMMC"\
								"\r\ndriver on FSP. J-Link RTT Viewer is used as user interface."\
...
#define FILEX_BLOCKMEDIA_MENU   "\r\nFileX block media Main Menu options:"\
                                "\r\n1. Enter 1 to format "CARD_TYPE" card."\
                                "\r\n2. Enter 2 to create a directory on "CARD_TYPE" card after checking available "\
                                "space."\
                                "\r\nCreate a text file inside the directory, write and read pre-defined data."\
                                "\r\n3. Enter 3 to display "CARD_TYPE" card content."\
                                "\r\nUser Input:  "
