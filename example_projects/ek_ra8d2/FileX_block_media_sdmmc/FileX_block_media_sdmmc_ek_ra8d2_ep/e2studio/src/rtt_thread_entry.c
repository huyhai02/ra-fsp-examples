...
/*******************************************************************************************************************//**
 * @brief     This function receives messages from the user thread via tx_queue_receive,
 *            displays them to the RTT Viewer, and releases the allocated print message block.
 * @param[IN] None.
 * @retval    None.
 **********************************************************************************************************************/
static void process_rtt_op_msg(VOID)
{
    ...
    do
    {
        ...
        if(err == TX_SUCCESS)
        {
            ...
            /* Send newline and carriage return to flush the RTT output buffer */
            APP_PRINT("\r\n");
