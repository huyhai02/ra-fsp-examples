...
time_format_t g_set_time = {.month = RESET_VALUE, .date = RESET_VALUE, .year = RESET_VALUE,\
...
/* Write predefined data into the given file */
static UINT filex_blockmedia_sdmmc_write(FX_FILE * p_file, CHAR * p_file_name);

/* Read written data from the given file */
...
void filex_thread_entry(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        PRINT_ERR_STR("\r\nRM_FILEX_BLOCK_MEDIA_Open API failed.\r\n");
        ERROR_TRAP(err);
    }
    ...
    fx_ret_val = tx_event_flags_get(&my_event_flags_group, RM_BLOCK_MEDIA_EVENT_MEDIA_INSERTED,\
                                    TX_AND_CLEAR, &g_actual_events, TIMER_TICKS);
    if ((TX_SUCCESS == fx_ret_val) && (RM_BLOCK_MEDIA_EVENT_MEDIA_INSERTED == g_actual_events))
    {
        PRINT_INFO_STR("\r\n"CARD_TYPE" Card is inserted.");
    }
    else
    {
        err = FSP_ERR_TIMEOUT;
     ...
     * this can be used to add time stamping to the files on SD/MicroSD card */
    ...
    while (true)
    {
        fx_ret_val = tx_event_flags_get(&my_event_flags_group, RM_BLOCK_MEDIA_EVENT_MEDIA_REMOVED,\
                                        TX_AND_CLEAR, &g_actual_events, TX_NO_WAIT);
        if ((TX_SUCCESS == fx_ret_val) && (RM_BLOCK_MEDIA_EVENT_MEDIA_REMOVED == g_actual_events))
        {
            PRINT_INFO_STR("\r\n"CARD_TYPE" Card is removed. Please insert "CARD_TYPE" card to proceed.");
            /* Check if card inserted event is received or not and proceed further */
            fx_ret_val = tx_event_flags_get(&my_event_flags_group, RM_BLOCK_MEDIA_EVENT_MEDIA_INSERTED,\
                                            TX_AND_CLEAR, &g_actual_events, TX_WAIT_FOREVER);
            if ((TX_SUCCESS == fx_ret_val) && (RM_BLOCK_MEDIA_EVENT_MEDIA_INSERTED == g_actual_events))
            {
                PRINT_INFO_STR("\r\n"CARD_TYPE" Card is inserted.");
            }
        ...
        /* Perform file operations using block media SDMMC */
...
/*******************************************************************************************************************//**
 * @brief       Perform file operations using block media SDMMC.
 * @param[IN]   None.
...
static UINT filex_blockmedia_sdmmc_operation(void)
{
    ...
    switch (read_data)
    {
        case FORMAT_CARD:
        {
            status = tx_event_flags_get(&my_event_flags_group, RM_BLOCK_MEDIA_EVENT_MEDIA_REMOVED,\
                                        TX_AND_CLEAR, &g_actual_events, TX_NO_WAIT);
            ...
            {
                PRINT_INFO_STR("\r\nFAT32 formatting of "CARD_TYPE" card, as per user input.");
                /* Sleep briefly to yield CPU to RTT thread print message */
                tx_thread_sleep(1);

                /* Format SD/MicroSD card
        ...
        case FILE_CREATION:
        {
            ...
            else
            {
                ...
                if ((uint8_t)FX_SUCCESS != status)
                {
                    PRINT_ERR_STR("\r\nfilex_blockmedia_sdmmc_read failed.\r\n");
                    return status;
                }
            }
        ...
        case DISPLAY_CONTENT:
        {
            ...
            else
            {
                ...
                file_info_t file_info = {.attributes = {RESET_VALUE}, .directory_name = {RESET_VALUE},\
                                         .size = RESET_VALUE, .time_stamp = {.month = RESET_VALUE, .date = RESET_VALUE,\
                                                                             .year = RESET_VALUE, .hour = RESET_VALUE,\
                                                                             .min = RESET_VALUE, .sec = RESET_VALUE}
                };

                PRINT_INFO_STR("\r\nFile Name           Attribute       File Size   Date & Time of last modification");
                /* Extract first entry details in the current directory */
                status = fx_directory_first_full_entry_find(&g_fx_media0, file_info.directory_name,\
                                                            (UINT *)file_info.attributes, &file_info.size,\
                                                            &file_info.time_stamp.year, &file_info.time_stamp.month,\
                                                            &file_info.time_stamp.date, &file_info.time_stamp.hour,\
                ...
                do
                {
                    ...
                    status = fx_directory_next_full_entry_find(&g_fx_media0, file_info.directory_name,\
                                                               (UINT *)file_info.attributes, &file_info.size,\
                                                               &file_info.time_stamp.year, &file_info.time_stamp.month,\
                                                               &file_info.time_stamp.date, &file_info.time_stamp.hour,\
                                                               &file_info.time_stamp.min, &file_info.time_stamp.sec);
                ...
                /* Re-initialize the status since the error conditions are already handled */
                status = FX_SUCCESS;
            }
        }
        break;
...
static UINT filex_blockmedia_sdmmc_write(FX_FILE * p_file, CHAR * p_file_name)
{
    ...
    status = tx_event_flags_get(&my_event_flags_group, RM_BLOCK_MEDIA_EVENT_WAIT_END,\
                                TX_AND_CLEAR, &g_actual_events, TIMER_TICKS);
...
/*******************************************************************************************************************//**
 * @brief       This function performs read operation to file.
 * @param[IN]   p_file          Pointer to file.
 * @param[IN]   p_file_name     Pointer to file name.
...
/*******************************************************************************************************************//**
 * @brief       This function assigns month value based on compiler returned month.
...
static void assign_month_value(time_format_t * time, char * read_buffer)
{
    /* This compiler MACRO that had date value is processed using the following conversion,
     * where ASCII value is converted to integer value and month value is assigned */
...
/*******************************************************************************************************************//**
 * @brief       This function de-initializes opened media.
 * @param[IN]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void deinit_media(void)
