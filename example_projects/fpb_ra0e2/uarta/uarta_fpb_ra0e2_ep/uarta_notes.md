...
Supported RA board: FPB-RA0E1, FPB-RA0E2
* 1 x RA board.
...
* For FPB-RA0E2:
    * To use J-Link OB VCOM: Short the E46, E47 traces.
...
| configuration.xml > Stacks > g_uarta UART (r_uarta) > Properties > Settings > Property > Module g_uarta UART (r_uarta) > General > Channel | 0 | 0 | Use UARTA channel 0 to communicate with the terminal application on the host PC. |
...
| configuration.xml > Stacks > g_uarta UART (r_uarta) > Properties > Settings > Property > Module g_uarta UART (r_uarta) > Interrupts > Callback | NULL | uarta_callback | It is called from the interrupt service routine (ISR) upon UART transaction completion reporting the transaction status. |
...
2. Connect the RA board to the host PC via a Type-C USB cable.
