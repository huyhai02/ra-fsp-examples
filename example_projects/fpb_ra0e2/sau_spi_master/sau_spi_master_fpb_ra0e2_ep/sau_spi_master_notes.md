...
|-----------------------------------------|-------------------|----------------|------------|
| configuration.xml > Stacks > g_sau_spi SPI (r_sau_spi) > Properties > Settings > Property > Common > Enable Single Channel | Disable | 01 | Enable single channel to reduce code size if only channel 01 or 20 is to be configured for SAU SPI. |
| configuration.xml > Stacks > g_sau_spi SPI (r_sau_spi) > Properties > Settings > Property > Common > DTC Support | Disabled | Enabled | Enable DTC Support. |
| configuration.xml > Stacks > g_sau_spi SPI (r_sau_spi) > Properties > Settings > Property > Module g_sau_spi SPI (r_sau_spi) > Channel | 00 | 01 | Use SAU SPI Channel 01 to communicate with the digital thermometer module. |
...
|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
|-----------------------------------------|-------------------|----------------|------------|
| configuration.xml > Pins > Pin Selection > Ports > P2 > P207 > Pin Configuration > Mode | Disable | Output mode (Initial Low) | Use P207 as output to drive SS_N pin of slave device during transfer process. |
...
* Information can be displayed using either the SEGGER J-Link RTT Viewer or a serial terminal (UART) via J-Link OB VCOM, depending on availability. If J-Link OB VCOM is unsupported, the example project defaults to the SEGGER J-Link RTT Viewer; if supported, it defaults to the serial terminal (UART).
  ...
  * The RA boards support J-Link OB VCOM: FPB-RA0E1, FPB-RA0E2, FPB-RA0L1.
