...
**Note:**
* To display information, users can select between the SEGGER J-Link RTT Viewer and the Serial Terminal (UART) with J-Link OB VCOM. It is important to note that the user should only operate a single terminal application at a time to avoid conflicts or data inconsistencies. For instructions on how to switch between these options, please refer the first step in the "5. Verifying Operation" section of this file.
...
### Hardware ###
Supported RA boards: EK-RA6M5, EK-RA4C1, EK-RA4M2, EK-RA4M3, EK-RA6E2, EK-RA6M3, EK-RA6M3G, EK-RA6M4
...
**For all supported boards (Except for EK-RA6E2):**
...
**For EK-RA6E2: This board does not support QPI mode**

|   Module Property Path and Identifier   |   Default Value   |   Used Value   |   Reason   |
|-----------------------------------------|-------------------|----------------|------------|
| configuration.xml > Stacks > Properties > g_qspi QSPI Flash (r_qspi) > Settings > Property > Module g_qspi QSPI (r_qspi) > General > SPI Protocol | Extended SPI | Extended SPI | Select Extended SPI mode. |
| configuration.xml > Stacks > Properties > g_qspi QSPI Flash (r_qspi) > Settings > Property > Module g_qspi QSPI (r_qspi) > General > Address Bytes | 3 | 3 | Select 3 bytes address. |
...
![ep_info](images/EP_Information.png "EP Information")

The EP Menu

* The Main Menu:

![main_menu](images/Memu_main.png "Main Menu")

* The Flash Operations Menu:

![flash_menu](images/Menu_Flash_Operations.png "The Flash Operations Menu")

* The Directory Operations Menu:

![dir_menu](images/Menu_Dir_Operations.png "The Directory Operations Menu")

* The File Operations Menu:

![file_menu](images/Menu_File_Operations.png "The File Operations Menu")

Flash Operations

* Erase the entire Flash:

![erase_entire_flash](images/Flash_erase_entire.png "Erase the entire Flash")

* Erase the first 32 KB of the flash:

![erase_32kb_flash](images/Flash_erase_32KB.png "Erase the first 32 KB of the flash")

* Format the File System:

![format_file_system](images/Format_file_system.png "Format the File System")

* Go back to the Main Menu:

![flash_go_back](images/Flash_back_to_main_menu.png "Go back to the Main Menu")
...
![directory_create](images/Dir_create.png "Directory Create")

* Get Directory Properties:

![directory_get_properties](images/Dir_get_properties.png "Directory Get Properties")

* Delete Directory:

![directory_delete](images/Dir_delete.png "Directory Delete")

* Go back to the Main Menu:

![directory_go_back](images/Dir_back_to_main_menu.png "Go back to the Main Menu")
...
![file_create](images/File_open.png "Create an empty file or open an existed file")

* Write to the File:

![file_write](images/File_write.png "Write to the File")

* Read from the File:

![file_read](images/File_read.png "Read from the File")

* Delete File:

![file_delete](images/File_delete.png "Delete File")

* Go back to the Main Menu:

![file_go_back](images/File_back_to_main_menu.png "Go back to the Main Menu")
...
The board supports J-Link OB VCOM: EK-RA4C1, EK-RA6E2.
...
**For J-Link RTT Viewer**: Define `USE_VIRTUAL_COM=0` (When using the SEGGER J-Link RTT Viewer, the RTT block address may be required to observe the EP's operation. Find the address 
    for the _SEGGER_RTT variable in the .map file (e2studio\Debug or e2studio\Release).)
