...
* @param[in]  pvParameters      Thread specific parameters.
* @retval     None.
**********************************************************************************************************************/
void user_app_thread_entry(void *pvParameters)
{
    ...
    TransportInterface_t xTransportInterface = {RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE};
    ...
    HTTPRequestInfo_t xRequestInfo = {RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE,\
                                      RESET_VALUE, RESET_VALUE};
    /* Represents a response returned from an HTTP server */
    HTTPResponse_t xResponse = {RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE,\
                                RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE,\
                                RESET_VALUE, RESET_VALUE};
    /* Represents header data that will be sent in an HTTP request */
    HTTPRequestHeaders_t xRequestHeaders = {RESET_VALUE, RESET_VALUE, RESET_VALUE};
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** Failed in hal_littlefs_init() function **\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    {
        APP_PRINT("\r\nmbedtls_platform setup successfully");
    }

    /* Print the pre-initialization Ethernet IP configuration */
    ...
    /* Initialize HTTPS client with pre-signed URL */
        ...
        if (APP_CHECK_DATA)
        {
            ...
            {   
                case GET:
                {
                    ...
                    {
                        APP_PRINT("Failed to send HTTP %s request to %s%s: Error = %s.", xRequestInfo.pMethod,\
                                  HTTPS_HOST_ADDRESS, xRequestInfo.pPath, HTTPClient_strerror(httpsClientStatus));
                    }
                    #endif
                    if (HTTPSuccess != httpsClientStatus)
                    {
                    ...
                    else
                    {
                        ...
                        /* SynchronousrResponse body starts with the string in the format like
                         * [{\"id\":\"0ENQG7RYQA40W17G2A2SFH8E9Q\",\"...\"}]".
                         * So, to fetch the ID_KEY other portion of string is avoided */

                        /* Set the flag to avoid GET call in the PUT request */
                        is_get_called = true;
                    }
                }
                    break;
                case PUT:
                {
                    ...
                    /* Check if PUT request called prior to the GET request and for updating the feed id in PUT
                     * request URL */
                    if (true != is_get_called)
                    {
                        if (true != ID_alive)
                        {
                            APP_PRINT("** ID for URL is not available. \
                                      We need to run GET method first to get ID **\r\n");
                            break;
                        }
                        else
                        {
                    ...
                    /* Format into string to send in JSON format */
...
eDHCPCallbackAnswer_t xApplicationDHCPHook(eDHCPCallbackPhase_t eDHCPPhase, uint32_t lulIPAddress)
{
    ...
    switch (eDHCPPhase)
    {
        ...
        default:
            /* Cannot be reached, but set eReturn to prevent compiler warnings where compilers are disposed to
             * generating one. */
        break;
    }
...
/*******************************************************************************************************************//**
 * @brief      User hooks for the Ping Reply. vApplicationPingReplyHook() is called by the TCP/IP
 *             stack when the stack receives a ping reply.
 * @param[in]  eStatus          Ping reply status.
 *             usIdentifier     Ping reply identifier.
 * @retval     None.
 **********************************************************************************************************************/
 ...
 * @param[in]  eNetworkEvent    Network event.
 * @retval     None.
 **********************************************************************************************************************/
...
void vApplicationIPNetworkEventHook(eIPCallbackEvent_t eNetworkEvent)
{
    if ((eNetworkUp == eNetworkEvent) && (dhcp_in_use))
    {
        ...
        /* The network is up and configured. Print out the configuration obtained from the DHCP server. */
...
BaseType_t provision_alt_key(void)
{
    ...
    ProvisioningParams_t params = {RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE};
 ...
 * @param[in]  NetworkContext               The output parameter to return the created network context.
...
HTTPStatus_t connect_aws_https_client(NetworkContext_t *NetworkContext)
{
    ...
    NetworkCredentials_t connConfig = {RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE,\
                                       RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE};
    TlsTransportParams_t xPlaintextTransportParams;
    memset(&xPlaintextTransportParams, RESET_VALUE, sizeof(TlsTransportParams_t));
    ...
    {
        TCP_connect_status = TLS_FreeRTOS_Connect(NetworkContext, HTTPS_HOST_ADDRESS, HTTPS_PORT, &connConfig,\
                                                  SOCKET_SEND_RECV_TIME_OUT_MS, SOCKET_SEND_RECV_TIME_OUT_MS);
 ...
 *             The user has to update their Active Key generated from io.adafruit.com site in the ACTIVE_KEY macro.
...
HTTPStatus_t add_header(HTTPRequestHeaders_t * pRequestHeaders)
{
    ...
    {
        Status = HTTPClient_AddHeader(pRequestHeaders, "X-AIO-Key", strlen("X-AIO-Key"), ACTIVE_KEY,\
                                      strlen(ACTIVE_KEY));
        if (Status != HTTPSuccess)
        {
            APP_ERR_PRINT("An error occurred at adding Active Key in HTTPClient_AddHeader() with error code: \
                           Error = %s.\r\n", HTTPClient_strerror(Status));
        }
    }
    else
    {
        APP_ERR_PRINT("An error occurred at adding json format in HTTPClient_AddHeader() with error code: \
                      Error = %s.\r\n", HTTPClient_strerror(Status));
    }
    return Status;
}
