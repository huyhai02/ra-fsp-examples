...
void terminal_thread_entry(void)
{
    ...
    status = (ULONG)tx_event_flags_set(&g_request_event, REQUEST_START_CONSOLE, TX_OR);
    if (TX_SUCCESS != status)
    {
 ...
 *  Description  : This function transfers a message to the PC based on the terminal frame taken from the terminal
 *                 output queue.
 *  Arguments    : None
