 ...
 *  Description  : This functions erases the flash memory using lower level flash APIs.
 *  Arguments    : None
 *  Return Value : FSP_SUCCESS     Upon successful operation
 *                 Any Other Error code apart from FSP_SUCCESS Unsuccessful operation
 **********************************************************************************************************************/
fsp_err_t erase_ospi_flash(void)
{
    ...
    /* Re-setup calibrate data after erase entire the OSPI Flash */
    err = ospi_b_setup_calibrate_data();
    RETURN_ERR_STR(err, "ospi_b_setup_calibrate_data FAILED \r\n");
...
static fsp_err_t ospi_b_setup_calibrate_data(void)
{
    ...
    if (RESET_VALUE != memcmp((uint8_t *)(OSPI_B_AUTO_CALIB_ADDRESS),
