...
#ifdef BOARD_RA0E3_FPB
            print_float(temp_buff, temperature);
            APP_PRINT("\r\nTemperature:%s %sC", temp_buff, DEGREE_SYMBOL);
#else
...
static fsp_err_t sensor_init(void)
{
    ...
    /* Wait for transfer complete event and handle error in the case of event failure */
    err = wait_for_sau_spi_event(SPI_EVENT_TRANSFER_COMPLETE);
