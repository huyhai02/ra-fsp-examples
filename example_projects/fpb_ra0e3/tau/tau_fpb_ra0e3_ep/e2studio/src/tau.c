...
static float divider_set(uint32_t square_wave_frequency) BSP_PLACE_IN_SECTION(".flash_gap");
static void tau_deinit(void) BSP_PLACE_IN_SECTION(".flash_gap");
static void handle_error(fsp_err_t err, uint8_t * err_str) BSP_PLACE_IN_SECTION(".flash_gap");
#else
...
#ifdef BOARD_RA0E3_FPB
static void print_float(char *p_buffer, float data, int enable_signal);
#endif
...
void tau_entry(void)
{
    ...
    while (true)
    {
        /* Start interval timer and delay timer mode */
 ...
 *  @retval     Any other error code apart from FSP_SUCCESS indicating unsuccessful operation.
 **********************************************************************************************************************/
static fsp_err_t tau_interval_and_delay_timer_operation(void)
{
    fsp_err_t err = FSP_SUCCESS;
 ...
 *               and counts blink times of LED2.
 *  @param[in]   None.
 *  @retval      FSP_SUCCESS    Upon successful operation.
 *  @retval      Any other error code apart from FSP_SUCCESS indicating unsuccessful operation.
 **********************************************************************************************************************/
...
#ifdef BOARD_RA0E3_FPB
    print_float(timer_buffer, input_pulse_min, 0);
#else
    ...
    while (true)
    {
        ...
        if (g_input_capture_detect_flag && g_interrupt_counter > ONE)
        {
            ...
            if (g_led2_status_flag)
            {
                ...
                input_pulse_frequency_value = (float)((float)(periodic_info.clock_frequency)
                                              / (float)(g_capture_value));
#ifdef BOARD_RA0E3_FPB
                print_float(timer_buffer, input_pulse_frequency_value, 0);
#else
        ...
        else if (g_external_counter_flag)
        {
            ...
            else
            {
                ...
                APP_ERR_RET(FSP_ERR_ASSERTION != FSP_SUCCESS,FSP_ERR_ASSERTION,
                            "External event count operation did not work as expected");
            }
...
/*******************************************************************************************************************//**
 *  @brief       This function is used to measures the low level pulse width of square wave, which is generated by
 *               another TAU channel.
 *  @param[in]   None.
 *  @retval      FSP_SUCCESS    Upon successful operation.
 *  @retval      Any other error code apart from FSP_SUCCESS indicating unsuccessful operation.
...
static fsp_err_t tau_measure_low_level_pulse_width_operation(void)
{
    ...
    while (DEMO_TIMES > g_measure_times)
    {
        ...
        if (g_measure_pulse_width_flag && g_interrupt_counter > ONE)
        {
            ...
            low_level_pulse_width = (float)(((float)(g_capture_value) /
                                            (float)(periodic_info.clock_frequency)) * CONVERT_SECOND_TO_MILISECOND);
#ifdef BOARD_RA0E3_FPB
            print_float(timer_buffer, low_level_pulse_width, 0);
#else
 ...
 *  @param[in]   None.
 *  @retval      square_wave_frequency    Frequency is set by user.
 **********************************************************************************************************************/
static uint32_t square_wave_frequency_set(void)
{
    fsp_err_t err                       = FSP_SUCCESS;
...
#ifdef BOARD_RA0E3_FPB
    print_float(timer_buffer, divided_output, 0);
#else
...
static fsp_err_t square_wave_period_set(void)
{
    ...
    raw_counts = (uint32_t)((periodic_info.clock_frequency * square_wave_period) /
                            (CONVERT_HALF_TO_ONE_CYCLE * CONVERT_SECOND_TO_MILISECOND));
    /* Set period value */
...
static uint32_t get_user_input(uint32_t min_value, uint32_t max_value)
{
    ...
    while (true)
    {
        ...
        /* Read the first byte of data provided by the user */
...
static uint32_t max_timer_calculate(tau_mode_t tau_mode)
{
    ...
    {
        /* Determine the maximum values of interval timer and delay counter corresponding
         * to the selected TAU clock source */
    ...
    {
        /* Determine the maximum period of square wave output sequence 3 corresponding to
...
static float min_timer_calculate(tau_mode_t tau_mode)
{
    ...
    {
        /* Determine the minimum frequency can be measured by input capture operation
         * corresponding to the selected TAU clock source */
 ...
 * @param[in]   p_args     Pointer to TAU timer callback arguments.
 * @return      None.
 **********************************************************************************************************************/
...
/*******************************************************************************************************************//**
 * @brief       Handle fatal errors by printing error information, de-initializing TAU modules,
 *              and trapping the system.
 * @param[in]   err       FSP error code.
 * @param[in]   err_str   Pointer to error message string.
 * @retval      None.
 **********************************************************************************************************************/
static void handle_error(fsp_err_t err, uint8_t * err_str)
{
...
/*******************************************************************************************************************//**
 * @brief       This function closes opened TAU module.
 * @param[in]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void tau_deinit(void)
...
#ifdef BOARD_RA0E3_FPB
/*******************************************************************************************************************//**
 * @brief       Format a floating-point value into a string without using floating-point printf,
 *              using integer arithmetic to reduce code size.
 * @param[in]   p_buffer        Pointer to the output buffer that stores the formatted string.
 * @param[in]   data            Floating-point value to be formatted.
 * @param[in]   enable_signal   Enable sign handling.
 *                              - 1: Prepend '-' for negative values.
 *                              - 0: Negative sign is suppressed.
 * @retval      None.
 **********************************************************************************************************************/
static void print_float(char *p_buffer, float data, int enable_signal)
{
    ...
    char *p   = p_buffer;
    char *end = p_buffer + STR_SIZE - 1;

    const int scale = pow10_table[DECIMALS];

    /* Scale and truncate */
    int scaled = (int)(data * (float)scale);
    ...
    int frac_part = scaled - int_part * scale;

    /* Integer part */
    int div = 1;
    while (int_part / div >= 10)
    {
        div *= 10;
    }
