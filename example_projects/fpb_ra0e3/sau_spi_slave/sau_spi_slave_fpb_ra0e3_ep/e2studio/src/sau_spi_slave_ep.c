...
#ifdef BOARD_RA0E3_FPB
        print_float(temp_buff, mcu_temp_c);
        APP_PRINT("\r\nMCU Temperature Reading from ADC:%s %sC", temp_buff, DEGREE_SYMBOL);
#else
...
{
    if (SAU_SPI_CHANNEL == p_args->channel)
    {
 ...
 *  Description  :  This function formats data without using floating-point print.
 *  Arguments    :  data    Data value needs to be formated.
                    *str    Pointer to buffer for formatted string.
 *  Return Value :  None.
 **********************************************************************************************************************/
static void print_float(char * str, float data)
{
