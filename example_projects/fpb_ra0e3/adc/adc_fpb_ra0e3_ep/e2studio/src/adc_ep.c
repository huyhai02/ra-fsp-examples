...
#endif /* BSP_PERIPHERAL_ADC_PRESENT */

/* Private function declarations */
/* Open the ADC module, configures and initiates the scan */
static fsp_err_t adc_scan_start(void);

/* Stop the ADC scan if the ADC is continuous scan or sequential mode, then close the module */
...
/* Track previous ADC value for deviation check */
static uint16_t g_prev_adc_data;
/* Check ADC deviation and determines if re-calibration is needed */
...
/* Format data without using floating-point print */
static void print_float(char * str, float data);
#endif
...
/*******************************************************************************************************************//**
 * @brief       This function opens the ADC, configures and starts the scan/the select.
 * @param[in]   None.
...
static fsp_err_t adc_scan_start(void)
{
    ...
    if (false == g_ready_to_read)
    {
        ...
        /* Indicate to start reading the ADC data */
 ...
 * @param[in]   None.
 * @retval      FSP_SUCCESS     Upon successful stops the ADC scan/select and closes the ADC.
...
static fsp_err_t adc_scan_stop(void)
{
    ...
    {
        APP_PRINT("\r\nStop command is not supported in One-shot mode, or "
                  "the user has not pressed Start Scan in Sequential mode mode\r\n");
    }
    ...
    else
    {
        APP_PRINT("\r\nStop command is not supported in Single Scan mode, or "
                  "the user has not pressed Start Scan in Continuous mode\r\n");
    }
    APP_PRINT("\r\nPress any other key (except 1 and 2) to go back to the main menu\r\n");
    return err;
#endif
...
fsp_err_t adc_read_data(void)
{
...
#elif defined BOARD_RA0E3_FPB
        g_adc_volt = (float)((g_adc_data * V_REF)/ADC_10_BIT);
        print_float(g_volt_str, g_adc_volt);
#else
    {
        g_adc_volt = (float)((g_adc_data * V_REF)/ADC_12_BIT);
        snprintf((char *)g_volt_str, sizeof(g_volt_str), "%0.2f", g_adc_volt);
    }
#endif /* BOARD_RA2A1_EK, BOARD_RA0E3_FPB */
     ...
     * avoid reading unnecessarily. Close the ADC module as it gets opened in start scan/select command. */
...
#endif /* (defined(BOARD_RA2A1_EK)) || (BSP_PERIPHERAL_ADC_B_PRESENT) */
...
#ifdef BOARD_RA0E3_FPB
/*******************************************************************************************************************//**
 *  @brief       This function formats data without using floating-point print.
 *  @param[in]   data          Data value needs to be formated.
 *  @param[out]  str           Pointer to buffer for formatted string.
 *  @retval      None.
 **********************************************************************************************************************/
static void print_float(char * str, float data)
{
     ...
     int data_frac = (int)(fabsf(data - (float)data_int) * powf(10.0f, DECIMALS));
     snprintf(str, STR_SIZE, "%d.%0*d", data_int, (int)DECIMALS, data_frac);
}
#endif
