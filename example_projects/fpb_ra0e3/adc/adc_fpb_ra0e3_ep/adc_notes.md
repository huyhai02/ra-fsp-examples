...
Note: FPB-RA0E1, FPB-RA0E2, MCK-RA6T2, EK-RA8P1, FPB-RA0L1, MCK-RA8T2, EK-RA8M2 EK-RA8D2, FPB-RA0E3 do not support Window Compare Mode.
...
| :-------------------------------------: | :---------------: | :------------: | :--------: |
| configuration.xml > Stacks > g_adc ADC (r_adc_d) > Properties > Settings > Property > Module g_adc ADC (r_adc_d) > General > Resolution | 10-Bit | 10-Bit | To select the conversion resolution for unit. |
...
| configuration.xml > Stacks > g_adc ADC (r_adc_d) > Properties > Settings > Property > Module g_adc ADC (r_adc_d) > General > Operation trigger | Wait | No-wait (Valid for Hardware start trigger source only) | To select the operation trigger mode. |
| configuration.xml > Stacks > g_adc ADC (r_adc_d) > Properties > Settings > Property > Module g_adc ADC (r_adc_d) > Input > A/D Input channel | Channel 0 | Channel 0 | To select Pin for ADC Channel - channel availability varies by MCU. |
...
The images below showcase the ADC output for Sequential mode on J-Link RTT Viewer:
...
The image below shows the ADC output for One-shot mode on J-Link RTT Viewer:
	
![ADC_One_Shot_Mode](images/ADC_One_Shot_Mode.jpg "RTT output - ADC_One_Shot_Mode")

## Special Topics ##

1) FPB-RA0E3 limitations: Enabling printing floats to the RTT Viewer is not required and should be disabled to avoid RAM overflow.
