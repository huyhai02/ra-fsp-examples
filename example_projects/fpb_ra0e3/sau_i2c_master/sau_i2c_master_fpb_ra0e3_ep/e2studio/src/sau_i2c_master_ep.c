...
#else
        /* Format the x_axis and print on RTT Viewer */
        print_float(float_str, x_axis);
        APP_PRINT("\r\nX-axis = %s g, ", float_str);

        /* Format the y_axis and print on RTT Viewer */
        print_float(float_str, y_axis);
        APP_PRINT("Y-axis = %s g, ", float_str);

        /* Format the y_axis and print on RTT Viewer */
        print_float(float_str, z_axis);
        APP_PRINT("Z-axis = %s g", float_str);
#endif
    }
}
...
static void sensor_deinit(void)
{
    ...
    /* Close the ICU module if it already opened */
    ...
    /* Close the SAU I2C module if it already opened */
...
static void handle_error(fsp_err_t err, const char * err_str)
{
    if (FSP_SUCCESS != err)
    {
        ...
        /* De-initialize hardware used by sensor */
        sensor_deinit();
...
static void print_float(char * str, float data)
{
    /* Apply zero threshold to filter out insignificant noise */
    if ((fabsf(data) + ROUND ) < ZERO_THRESHOLD) data = 0.0f;

    /* Determine sign for each data */
    char sign_data = (data < 0) ? '-' : ' ';

    /* Extract integer and fractional parts */
    float abs_data = fabsf(data);
    int data_int  = (int)abs_data;
    int data_frac = (int)((abs_data - (float)data_int) * powf(10.0f, DECIMALS));
    snprintf(str, STR_SIZE, "%c%d.%0*d", sign_data, data_int, (int)DECIMALS, data_frac);
