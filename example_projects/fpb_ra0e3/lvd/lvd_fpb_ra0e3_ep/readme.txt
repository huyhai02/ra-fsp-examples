...
1. Project Overview:
	...
	LVD0 is configured in OFS1 or OFS1_SEC register in configuration.xml > BSP
...
2. Hardware Requirements:
	...
	1 x External J-Link Segger with 9 pins (19 pins for MCK-RA8T1, FPB-RA0E1) connector and Type-B USB to connect
	    J-Link with PC (e.g., https://www.mouser.com/ProductDetail/Segger-Microcontroller/8.08.00?qs=sGAEpiMZZMuRZxwUfDU0mj7SZp0j2IIkjt1vgBTHRyw%3D).
...
3. Hardware Connections:
	...
	For FPB-RA0L1, FPB-RA0E3:
		Note: For FPB-RA0L1, the user must place jumper J9 on pins 2-3 to use external debug.
		      For FPB-RA0E3, the user must turn ON S3 to use external debug.
		Connect External J-Link connector ---> J13 JTAG point (Near Debug USB).
		LVD1 generates an interrupt when input voltage raises above 2.91V.
		LVD0 resets the MCU when input voltage drops below 2.62V.
...
3) For EK-RA8M1, EK-RA8D1, MCK-RA8T1, FPB-RA8E1, EK-RA8E2, EK-RA8P1, EK-RA8M2, EK-RA8D2 after changing input voltage
   values from below low threshold (ex: 2.8V) to above high threshold (ex: 3.3V), re-connect J-Link RTT Viewer
   to show log.
...
5) For EK-RA8P1, EK-RA8M2, EK-RA8D2: When using SEGGER J-Flash Lite to program the hex file, select the correct flash
