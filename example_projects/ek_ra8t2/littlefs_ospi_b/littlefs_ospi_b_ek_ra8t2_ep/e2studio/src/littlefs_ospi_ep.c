/***********************************************************************************************************************
 * File Name    : littlefs_ospi_ep.c
 * Description  : Contains macros data structures and functions used in littlefs_ospi_ep.c and hal_entry.c.
 **********************************************************************************************************************/
/***********************************************************************************************************************
 ...
 **********************************************************************************************************************/
static int32_t littlefs_ospi_init(void);
static void littlefs_ospi_deinit(void);
static uint8_t get_user_input(void);
static uint8_t handle_user_request(void);
static int32_t littlefs_ospi_operation(void);
...
void littlefs_ospi_entry(void)
{
    ...
    APP_PRINT("\r\nOSPI initialized successfully!\r\n");

    /* Initialize LittleFS for OSPI */
    lfs_err = littlefs_ospi_init();
    if (LFS_ERR_OK != lfs_err)
    {
        APP_ERR_PRINT("\r\nlittlefs_ospi_init failed!!!\r\n");
        APP_ERR_TRAP(lfs_err);
    }
    APP_PRINT("\r\nLittleFS OSPI initialized successfully!\r\n");
    ...
    while (true)
    {
        ...
        lfs_err = littlefs_ospi_operation();
        if (LFS_ERR_OK != lfs_err)
        {
            APP_ERR_PRINT("littlefs_ospi_operation failed!!!\r\n");
            littlefs_ospi_deinit();
            APP_ERR_TRAP(lfs_err);
        }
    }
}
/***********************************************************************************************************************
* End of function littlefs_ospi_entry
***********************************************************************************************************************/

/***********************************************************************************************************************
 *  Function Name: littlefs_ospi_operation
...
static int32_t littlefs_ospi_operation(void)
{
    ...
    if (RESET_VALUE != user_request)
    {
        ...
        switch (user_request)
        {
            case REQUEST_ERASE_ENTIRE_OSPI:
            {
                /* Erase the entire OSPI flash */
                err = erase_ospi_flash(SPI_FLASH_ERASE_SIZE_CHIP_ERASE);
                lfs_err = (int32_t) err;
                APP_ERR_RET(lfs_err != LFS_ERR_OK, lfs_err, "erase_ospi_flash failed!!!\r\n");
                break;
            }
            case REQUEST_ERASE_32KB_OSPI:
            {
                /* Erase the first 32 KB of the OSPI flash */
                err = erase_ospi_flash(OSPI_B_32K_SIZE);
                lfs_err = (int32_t) err;
                APP_ERR_RET(lfs_err != LFS_ERR_OK, lfs_err, "erase_ospi_flash failed!!!\r\n");
                break;
            }
...
/***********************************************************************************************************************
* End of function littlefs_ospi_operation
***********************************************************************************************************************/

/***********************************************************************************************************************
 *  Function Name: littlefs_ospi_init
 *  Description  : This function initializes LittleFS for OSPI.
 *  Arguments    : None.
 *  Return Value : LFS_ERR_OK    Upon successful operation.
 *                 Any other error code apart from LFS_ERR_OK.
 **********************************************************************************************************************/
...
/***********************************************************************************************************************
* End of function littlefs_ospi_init
***********************************************************************************************************************/

/***********************************************************************************************************************
 *  Function Name: littlefs_ospi_deinit
 *  Description  : This function de-initializes LittleFS for OSPI flash.
 *  Arguments    : None.
 *  Return Value : None.
 **********************************************************************************************************************/
static void littlefs_ospi_deinit(void)
...
/***********************************************************************************************************************
* End of function littlefs_ospi_deinit
***********************************************************************************************************************/
...
static int32_t dir_create(void)
{
    ...
    memset(&dir_info, RESET_VALUE, sizeof(dir_info));
...
static int32_t file_open(void)
{
    ...
    memset(&file_info, RESET_VALUE, sizeof(file_info));
    ...
    switch (g_menu)
    {
        ...
        case MENU_FLASH:
        {
            switch (user_input)
            {
                ...
                {
                    user_request = REQUEST_ERASE_ENTIRE_OSPI;
                    break;
                }
                case SELECT_ERASE_32KB_FLASH:
                {
                    user_request = REQUEST_ERASE_32KB_OSPI;
                    break;
                }
...
/***********************************************************************************************************************
 *  Function Name: g_rm_littlefs_spi_flash_callback
 *  Description  : User defined callback.
 *  Arguments    : p_args   Callback arguments.
 *  Return Value : None.
 **********************************************************************************************************************/
void g_rm_littlefs_spi_flash_callback(rm_littlefs_spi_flash_callback_args_t *p_args)
{
    ...
    if (p_args->event == RM_LITTLEFS_SPI_FLASH_EVENT_BUSY)
    {
        /* Do nothing */
    }
}
/***********************************************************************************************************************
* End of function g_rm_littlefs_spi_flash_callback
***********************************************************************************************************************/
