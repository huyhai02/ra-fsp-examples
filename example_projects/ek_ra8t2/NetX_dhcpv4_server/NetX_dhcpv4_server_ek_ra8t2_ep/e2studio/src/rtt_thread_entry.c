...
void rtt_thread_entry(void)
{
    ...
    if (TX_SUCCESS != err)
    {
        APP_PRINT("\r\nERROR: RTT framework initialize failed\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    do
    {
        ...
        if (err == TX_SUCCESS)
        {
            ...
            switch (rtt_op_data->msg_id)
            {
                ...
                {
                    /* Print DHCPv4 Server menu options */
                    APP_PRINT(DHCPV4_SERVER_MENU);
                }
                break;

                case RTT_OUTPUT_MESSAGE_APP_CLIENT_NUM:
                {
                    /* Assign lease time to local buffer */
                    UINT clients_assigned = *(UINT*)rtt_op_data->p_msg;
                    APP_PRINT("\r\nNumber of IP leased upon power up: %d\r\n", clients_assigned);
                }
                break;

                case RTT_OUTPUT_MESSAGE_APP_PRINT_CLIENT_NAME:
                {
                    APP_PRINT("\r\nClient's name: %s\r\n", rtt_op_data->p_msg);
                }
                break;
