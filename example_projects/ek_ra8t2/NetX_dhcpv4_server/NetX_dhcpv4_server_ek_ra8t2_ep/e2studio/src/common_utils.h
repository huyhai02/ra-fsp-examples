...
#define MODULE_NAME          "NetX_dhcpv4_server"
#define BANNER_INFO          "\r\n********************************************************************************"\
...
#else
/* ONLY 50% of POOL memory can be used for output buffer for example - 256 bytes is pool allocated
 * and SEGGER defined BUFFER_SIZE_UP macro is 1024
 * hence only 128 bytes can be used for output buffer maximum so formula made here is
 * 1) (1024/256) = 4
 * 2)  4*32 = 128 */
...
typedef enum event_id
{
    ...
    RTT_OUTPUT_MESSAGE_APP_CLIENT_NUM,
