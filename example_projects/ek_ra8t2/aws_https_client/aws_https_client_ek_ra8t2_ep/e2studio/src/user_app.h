...
/* The user has to update according to their LAN/WAN test setup for testing of ping IP address */
...
/* @brief To get the most recent value. Get API from GET url
 * (https://ioadafruit.com/api/v2/{username}/feeds/{feed_key}/data?limit=1):
 * /api/v2/{username}/feeds/{feed_key}/data?limit=1 **/
#define HTTPS_GET_API                           "/api/v2/canhtran/feeds/adc/data?limit=1"

/* HTTPS_PUT_POST_API can be used in PUT and POST methods.
 * PUT method will update data point at requested <id>.
 * PUT url: https://ioadafruit.com/api/v2/{username}/feeds/{feed_key}/data/{id},
 * where the <id> will append to the url in the process of
...
#define ACTIVE_KEY                              "aio_cbza39te4N0wokMJCxS6eNl8Zfmp"
...
#define CLIENT_CERTIFICATE_PEM  "-----BEGIN CERTIFICATE-----\n"\
...
#define CLIENT_KEY_PEM          "-----BEGIN PRIVATE KEY-----\n"\
...
#define HTTPS_TRUSTED_ROOT_CA   "-----BEGIN CERTIFICATE-----\n"\
...
#define EP_INFO     "\r\nThis project demonstrates the basic functionality of AWS https client library with"\
		            "\r\nEthernet driver on Renesas RA MCUs based on Renesas FSP using FreeRTOS. AWS client"\
		            "\r\nis used to connect to https adafruit server which is cloud platform. On successful"\
                    "\r\nconnection, menu is displayed enabling the user to send GET, PUT, POST requests to"\
                    "\r\nadafruit server. On POST/PUT request, ADC output is read and uploaded to adafruit server."\
                    "\r\nOn GET request, the last updated data is received from adafruit. J-Link RTT Viewer is"\
...
typedef enum Userinput
{
    ...
    /* DHCP has an option for clients to register their host name. It doesn't
