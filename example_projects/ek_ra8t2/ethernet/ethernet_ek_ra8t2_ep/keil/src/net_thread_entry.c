...
uint8_t *gp_remote_ip_address = (uint8_t *) USR_TEST_PING_IP;
    ...
    /* DHCP populates these IP address, Subnet mask and Gateway Address. So start with this is zeroed out values
...
uint32_t ulRand()
{
    ...
    uint32_t ulResult = ((((uint32_t) rand()) & 0x7fffuL)) | ((((uint32_t) rand()) & 0x7fffuL) << 15) |
...
void net_thread_entry(void *pvParameters)
{
    ...
    /* Print the Ethernet configuration prior to the IP initializes */
...
#if (ipconfigUSE_DHCP != 0)
/*******************************************************************************************************************//**
* @brief      This is the user hook for the DHCP response. xApplicationDHCPHook() is called by DHCP client code
...
void print_pingResult(void)
{
    ...
    APP_PRINT("\r\nPackets: Sent = %02d, Received = %02d, Lost = %02d\r\n", ping_data.sent,\
              ping_data.received, ping_data.lost);
}
...
void print_ipconfig(void)
{
    ...
    APP_PRINT("\tSubnet Mask . . . . . . . . . . . : %d.%d.%d.%d\r\n", ucNetMask[0], ucNetMask[1],\
                                                                       ucNetMask[2], ucNetMask[3]);
