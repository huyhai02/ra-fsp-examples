...
fsp_err_t spi_init(void)
{
...
#endif /* BSP_PERIPHERAL_SPI_B_PRESENT */
    /* Handle error */
    if (FSP_SUCCESS != err)
    {
        /* SPI Master failure message */
        APP_ERR_PRINT("** R_"SPI_TYPE"_Open API for SPI Master failed **\r\n");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        APP_ERR_PRINT("** R_"SPI_TYPE"_Open API for SPI Slave failed **\r\n");
...
#endif /* BSP_PERIPHERAL_SPI_B_PRESENT */
        {
            ...
            APP_ERR_PRINT("** R_"SPI_TYPE"_Close API for SPI Master failed **\r\n");
        }
    }
    return err;
}
...
/*******************************************************************************************************************//**
 * @brief       This function demos both R_SPI_Write()/R_SPI_B_Write() and R_SPI_Read()/R_SPI_B_Read() individually.
 * @param[IN]   None.
...
fsp_err_t spi_write_and_read(void)
{
    ...
    APP_PRINT("\r\nEnter text input for Master buffer. Data size should be less than 64 bytes.\r\n");
    ...
    if (err != FSP_SUCCESS)
    {
        APP_ERR_PRINT("\r\nSlave R_"SPI_TYPE"_Read() failed");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nMaster R_"SPI_TYPE"_Write() failed");
        return err;
    }
    ...
    g_master_event_flag = (spi_event_t) RESET_VALUE;  /* Reset master_event flag */
    g_slave_event_flag  = (spi_event_t) RESET_VALUE;  /* Reset slave_event flag */
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nSlave R_"SPI_TYPE"_Write() failed");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nMaster R_"SPI_TYPE"_Read() failed");
        return err;
    }
...
/*******************************************************************************************************************//**
 * @brief       This function demos R_SPI_WriteRead()/R_SPI_B_WriteRead() for both Master and Slave simultaneously.
 * @param[IN]   None.
...
fsp_err_t spi_write_read(void)
{
    ...
    memset(&g_master_tx_buff[0], NULL_CHAR, (BUFF_LEN * BITS_TO_BYTES));
    memset(&g_master_rx_buff[0], NULL_CHAR, (BUFF_LEN * BITS_TO_BYTES));
    memset(&g_slave_tx_buff[0], NULL_CHAR, (BUFF_LEN * BITS_TO_BYTES));
    memset(&g_slave_rx_buff[0], NULL_CHAR, (BUFF_LEN * BITS_TO_BYTES));
    ...
    APP_PRINT("\r\nEnter text input for Slave buffer. Data size should be less than 64 bytes.\r\n");
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nSlave R_"SPI_TYPE"_WriteRead() failed");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nMaster R_"SPI_TYPE"_WriteRead() failed");
        return err;
    }
...
fsp_err_t spi_exit_demo(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nMaster R_"SPI_TYPE"_Close() failed");
        return err;
    }
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\nSlave R_"SPI_TYPE"_Close() failed");
    }
    return err;
}
...
void spi_clean_up(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        APP_ERR_PRINT("** R_"SPI_TYPE"_Close API for master failed **\r\n");
    }
    ...
    {
        /* SPI Close failure message */
        APP_ERR_PRINT("** R_"SPI_TYPE"_Close API for slave failed **\r\n");
    }
}
