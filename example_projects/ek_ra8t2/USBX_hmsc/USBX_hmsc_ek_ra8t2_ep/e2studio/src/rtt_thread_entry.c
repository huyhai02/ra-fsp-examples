...
void rtt_thread_entry(void)
{
    ...
    while (1)
    {
        ...
        process_rtt_op_msg();
#if (BSP_CFG_RTOS == AZURE_RTOS)
        /* Sleep briefly to yield CPU to higher priority thread */
        tx_thread_sleep(1);
#endif /* BSP_CFG_RTOS == AZURE_RTOS */
    }
}
    ...
    do
    {
        ...
        if (err == TX_SUCCESS)
        {
            ...
            switch (rtt_op_data->msg_id)
            {
                ...
                {
                    /* Print USBX HMSC menu options */
                    APP_PRINT(USBX_HMSC_MENU);
                }
                break;
                ...
                break;
                case RTT_OUTPUT_MESSAGE_APP_PRINT_VOLUME_SPACE:
                {
                    ULONG volume_space = *(ULONG*)rtt_op_data->p_msg;
                    APP_PRINT("\r\nAvailable media volume is: %s bytes\r\n", volume_space);
                }
                break;
