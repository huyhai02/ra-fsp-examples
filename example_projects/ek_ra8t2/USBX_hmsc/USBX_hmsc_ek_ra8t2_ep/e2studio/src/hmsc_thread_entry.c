/***********************************************************************************************************************
 * File Name    : hmsc_thread_entry.c
 * Description  : Contains data structures and functions used in hmsc_thread_entry.c.
 **********************************************************************************************************************/
/***********************************************************************************************************************
...
static time_format_t g_set_time = {.month = RESET_VALUE, .date = RESET_VALUE, .year = RESET_VALUE,\
 ...
 * @param[in] instance     Pointer to the specific host class object that the function handles for the given event.
 * @retval    None.
 **********************************************************************************************************************/
static UINT apl_change_function(ULONG event, UX_HOST_CLASS * host_class, VOID * instance)
{
    ...
    if (UX_SUCCESS == _ux_utility_memory_compare(_ux_system_host_class_storage_name, host_class,
...
void hmsc_thread_entry(void)
{
    ...
    PRINT_INFO_STR("USB initialized completed");

    /* Extract date and time from compiler and add the time stamp to the files on USB flash drive */
    time_stamping();

    PRINT_INFO_STR("Date and time is set for FileX as per compiler DATE and TIME values");

    PRINT_INFO_STR("Insert the USB flash drive");
    ...
    {
        PRINT_INFO_STR("USB flash drive is inserted");
    }
    ...
    while (true)
    {
        ...
        {
            PRINT_INFO_STR("Connect USB flash drive for EP to work");
            actual_flags = RESET_VALUE;
            /* Wait until device inserted */
            tx_event_flags_get(&g_usb_plug_events, EVENT_USB_PLUG_IN, TX_AND_CLEAR, &actual_flags, TX_WAIT_FOREVER);
            if (EVENT_USB_PLUG_IN == actual_flags)
        ...
        /* Below section of code (handling and processing user input) will execute when USB flash drive is inserted */
        else
        {
            ...
            if (TX_SUCCESS == err)
            {
                ...
                if (FX_SUCCESS != status)
                {
                    ...
                    {
                        PRINT_ERR_STR("USB flash drive is plugged out please insert the USB flash drive");
                    }
                    else
                    {
 ...
 * @retval      FX_SUCCESS      USB operation completed successfully.
 * @retval      Any other error code apart from FX_SUCCESS  USB operation failed.
 **********************************************************************************************************************/
static UINT usbx_hmsc_operation(uint8_t read_data)
{
    UINT status = RESET_VALUE;
    ...
    /* Print the banner only if USB flash drive is inserted */
    if (EVENT_USB_PLUG_OUT != actual_flags)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_PRINT_MENU, RESET_VALUE, NULL);
    }
...
static UINT usbx_hmsc_file_directory_creation(void)
{
    ...
    if (EVENT_USB_PLUG_OUT == actual_flags)
    {
        PRINT_INFO_STR("USB flash drive is removed. Please insert USB flash drive to proceed");
    }
    else
    {
...
/*******************************************************************************************************************//**
 * @brief       This function displays the directory content of current directory.
 * @param[in]   None.
...
static UINT usbx_display_working_directory(void)
{
    ...
    file_info_t file_info = {.directory_name = {RESET_VALUE},
                             .attributes = {RESET_VALUE},
                             .size = RESET_VALUE,
                             .time_stamp = {.month = RESET_VALUE, .date = RESET_VALUE, .year = RESET_VALUE,\
                                            .hour = RESET_VALUE, .min = RESET_VALUE, .sec = RESET_VALUE}
                            };
    ...
    else
    {
        ...
        /* Check and print the file, directory details until the last entry */
        do
        {
    ...
    /* Return FSP_SUCCESS since the error conditions are already handled */
    return FSP_SUCCESS;
}
...
static UINT usbx_safely_eject(void)
{
    ...
    /* Wait for USB flash drive to be plugged out */
...
static void deinit_media(void)
{
    ...
    /* Un-initialize all the host code for USB */
 ...
 * @param[in]   *read_buffer    Pointer to the buffer that stores the captured time and data format.
 * @retval      None.
 **********************************************************************************************************************/
static void assign_month_value(time_format_t * time, char * read_buffer)
{
...
/*******************************************************************************************************************//**
 * @brief       This function is used to add time stamp to the files on USB flash drive.
 * @param[in]   None.
 * @retval      None.
 **********************************************************************************************************************/
static void time_stamping(void)
