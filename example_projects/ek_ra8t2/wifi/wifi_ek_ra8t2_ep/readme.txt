...
4. Hardware Connections:
	...
	For EK-RA2L1, FPB-RA4E1, FPB-RA6E1, EK-RA8M1, EK-RA8D1, EK-RA8P1, EK-RA8E2, EK-RA8D2, EK-RA8M2, EK-RA8T2:
...
Note:
1) Before performing test, please confirm that the host PC has enabled the inbound rule 'File and Printer Sharing
   (Echo Request - ICMPv4-In)' for accept ping request from RA MCU to TCP server: Windows Defender Firewall
   -> Advanced settings -> Inbound Rules -> Enable File and Printer Sharing (Echo Request - ICMPv4-In).

2) For EK-RA2E2, scanning wifi access points does not work. Please connect to wifi access points by entering its
   credentials manually.

3) For EK-RA4M2, EK-RA4M3, EK-RA6M4, EK-RA6M5, FPB-RA4E1, FPB-RA6E1, MCK-RA6T2, EK-RA8M1, EK-RA8D1, MCK-RA8T1,
   EK-RA2A2, FPB-RA0E1, FPB-RA8E1, EK-RA4L1, FPB-RA0E2, EK-RA2L2, EK-RA8P1, EK-RA8E2, EK-RA4C1, FPB-RA0L1, EK-RA8D2,
   EK-RA8M2, EK-RA8T2 provide RTT Block Address in J-Link RTT Viewer to run the EP.

4) Sokit setup on PC as TCP server:
	...
	l. While sending, select the IP address of the client i.e. the MCU from the 'Connections' GroupBox. And then
	   click 'Send' button of the appropriate buffer to send message.
