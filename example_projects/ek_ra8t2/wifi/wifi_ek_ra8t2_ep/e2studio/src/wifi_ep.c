 ...
 * @param[in]   user_buff   Char pointer pointing to the user input buffer.
 * @retval      None.
 * @retval      None.
 **********************************************************************************************************************/
void user_input(char * user_buff)
{
    ...
    uint32_t num_bytes;             /* Number of bytes read by RTT Viewer */
...
fsp_err_t scan_and_select(void)
{
    ...
    do
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("\r\n** rm_wifi_onchip_silex_scan API failed **\r\n");
            return err;
        }

        /* Print SSID of WiFi APs */
        ...
        /* The user selects the SSID from WiFi APs list */
    ...
    {
        /* The user inputs the password of the WiFi AP */
...
/*******************************************************************************************************************//**
 * @brief       This function takes the user input for WiFi AP's SSID, security type and
...
fsp_err_t manual_connect(void)
{
    ...
    /* The user inputs the SSID of the WiFi AP */
    APP_PRINT("\r\nEnter SSID of WiFi Access Point:");
    user_input(g_wifi.ssid);

    /* The user selects the security type of the WiFi AP */
    do
    {
    ...
    /* Check for the Open security type */
    ...
    /* Connect to the user entered SSID */
...
fsp_err_t dns_query(char * p_dns, uint8_t * p_ip_addr)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\n** rm_wifi_onchip_silex_dns_query API failed **\r\n");
        return err;
    }
...
fsp_err_t ping(uint8_t * p_ip_addr)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("\r\n** rm_wifi_onchip_silex_ping API failed **\r\n");
...
/*******************************************************************************************************************//**
 * @brief       This function creates a new TCP socket in client mode.
 * @param[in]   None.
...
/*******************************************************************************************************************//**
 * @brief       This function connects to the TCP server IP address.
...
fsp_err_t tcp_socket_connect(uint32_t ip_addr, uint32_t port)
{
    ...
    APP_ERR_PRINT("\r\n** rm_wifi_onchip_silex_tcp_connect API failed **\r\n");
    return err;
}

/*******************************************************************************************************************//**
 * @brief       This function sends data to the TCP server.
...
/*******************************************************************************************************************//**
 * @brief       This function receives data from the TCP server.
...
void wifi_deinit(void)
{
    ...
    if (FSP_SUCCESS == g_wifi.status)
    {
        /* If the WiFi connected, closing the WiFi module. Thus disconnecting from connected AP */
        err = rm_wifi_onchip_silex_close();
        /* Handle error */
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("\r\n** rm_wifi_onchip_silex_close API failed **\r\n");
            APP_ERR_TRAP(err);
        }
    }
}
