...
static fsp_err_t sdram_transmission(uint32_t start_addr, sdram_trans_dir_t dir)
{
    ...
    APP_ERR_RET(FSP_SUCCESS != err, err, "**R_DMAC_SoftwareStart API failed**\r\n");
    ...
    {
        APP_ERR_RET(FSP_SUCCESS != err, FSP_ERR_TIMEOUT, "\r\nWrite operation timeout\r\n");
    }
...
static fsp_err_t sdram_verify_data(void)
{
    ...
    for (uint32_t i = 0; i < SDRAM_SIZE / NUM_ADDR_FOR_BLOCK; ++i)
    {
        ...
        APP_ERR_RET(FSP_SUCCESS != err, err, "sdram_transmission failed\r\n");
        ...
        APP_ERR_RET(RESET_VALUE != ret, FSP_ERR_INVALID_DATA, "Data is mismatch\r\n");
    }
    return FSP_SUCCESS;
}
/***********************************************************************************************************************
...
static fsp_err_t sdram_self_refresh(void)
{
    ...
    APP_ERR_RET(FSP_SUCCESS != err, err, "**R_ULPT_Start API failed**\r\n");

    /* Enter LPM mode */
    err = enter_lpm();
    APP_ERR_RET(FSP_SUCCESS != err, err, "erter_lpm failed\r\n");

    /* Stop timer */
    err = R_ULPT_Stop(&g_timer_cancel_lpm_ctrl);
    APP_ERR_RET(FSP_SUCCESS != err, err, "**R_ULPT_Stop API failed**\r\n");
    ...
    /* Verify written data */
    err = sdram_verify_data();
    APP_ERR_RET(FSP_SUCCESS != err, err, "sdram_verify_data failed\r\n");
...
#if (USE_VIRTUAL_COM == 1)
#if defined(BOARD_RA8D1_EK) || defined(BOARD_RA8E2_EK) || defined(BOARD_RA8P1_EK) || defined(BOARD_RA8D2_EK)\
    || defined(BOARD_RA8T2_EK)
