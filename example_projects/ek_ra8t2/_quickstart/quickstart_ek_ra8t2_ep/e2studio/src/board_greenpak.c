...
/* Slave address for GreenPak RAM access */
#define BSP_I2C_SLAVE_ADDR_GREENPAK_RAM    (0x20)

/* Slave address for GreenPak NVRAM access */
#define BSP_I2C_SLAVE_ADDR_GREENPAK_NVRAM  (0x21)

/* Slave address for GreenPak EEPROM access */
#define BSP_I2C_SLAVE_ADDR_GREENPAK_EEPROM (0x22)
 ...
 * Description  : Initialise connection to greenpack sw4 (programmable configurable mixed-signal device)
 * Argument     : None
...
fsp_err_t board_greenpak_init(void)
{
    ...
    {
        if (R_IIC_MASTER_Open (&g_board_i2c_master_ctrl, &g_board_i2c_master_cfg) != FSP_SUCCESS)
        {
            return (retval);
        }
    }
    ...
    R_IIC_MASTER_SlaveAddressSet(&g_board_i2c_master_ctrl, BSP_I2C_SLAVE_ADDR_GREENPAK_EEPROM,
    ...
    if ((GREENPAK_IO4_EXPECTED_VALUE != t0) || (GREENPAK_IO4_EXPECTED_VALUE != t1))
    {
        ...
        R_IIC_MASTER_SlaveAddressSet(&g_board_i2c_master_ctrl, BSP_I2C_SLAVE_ADDR_GREENPAK_RAM,
                I2C_MASTER_ADDR_MODE_7BIT);
    ...
    {
        /* No update required */
        sprintf(g_greenpak_warning_str, "\r\n");
        retval = FSP_SUCCESS;
    }
    return (retval);
}
