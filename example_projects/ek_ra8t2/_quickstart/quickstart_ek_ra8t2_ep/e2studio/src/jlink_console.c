...
static void jlink_console_write(const char_t *buffer)
{
    ...
    /* Cast to uint8_t *: buffer is a char_t string being passed to UART write which
      * accepts uint8_t pointer. All char_t values map directly to uint8_t range as
      * both represent single byte values.
      * Cast to uint32_t: strlen() returns size_t which may differ in width from uint32_t.
      * The buffer length is guaranteed to be within uint32_t range as it represents
      * a console output string of bounded length */
    ...
    switch (p_args->event)
    {
        ...
        case UART_EVENT_RX_CHAR:
        {
            ...
            if ((sizeof(s_out_of_band_received)) > s_g_out_of_band_index)
            {
                ...
                {
                    /* Cast to uint8_t: p_args->data is verified to be 8-bit data or less
                     * by the preceding check against UART_DATA_BITS_8, ensuring the value
                     * fits within uint8_t range (0-255) without loss of data */
