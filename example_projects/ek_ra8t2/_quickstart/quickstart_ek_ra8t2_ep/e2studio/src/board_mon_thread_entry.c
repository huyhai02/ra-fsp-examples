...
static void test_temperature_change()
{
    ...
    {
        /* Cast to int32_t to ensure signed arithmetic for calibration data */
        ...
        /* Cast int32_t to float32_t for floating-point arithmetic;
         * multiply by 1.0 to explicitly confirm float conversion */
        ...
        /* Compute temperature in degrees Celsius using the formula:
         * T = ((Vs - V1) / slope) + T_ref
         * Cast reference temperature to float32_t to ensure entire expression remains in floating-point */
        float32_t temperature_cf  = ((fvs_uv - fv1_uv) / fslope_uv_per_c) + \
                (float32_t)ADC_EXAMPLE_REFERENCE_CALIBRATION_TEMPERATURE;
 ...
 * Description  : Thread entry for DUT system status monitoring called by FSP.
 ...
 * Description  : SW1 Interrupt handler irq 13 Deep Sleep.
 * Argument     : p_args
 * Return Value : None
 *********************************************************************************************************************/
