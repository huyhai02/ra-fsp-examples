...
static bool_t   s_b_system_up = false;
...
fsp_err_t common_init(void)
{
...
#ifdef SDRAM_REQUIRED
    if (is_sdram_initialised() == false)
    {
        bsp_sdram_init ();
    }
#endif /* SDRAM_REQUIRED */
    ...
    g_board_status.led_intensity = 1;
    ...
    /* R_GPT_DutyCycleSet (s_pwm_pins[0].p_timer->p_ctrl, (uint32_t)1, s_pwm_pins[0].pin); */
 ...
 * Description  : user callback for 'blinking' blue led, invoked by FSP
 * Argument     : p_args
 * Return Value : .
 *********************************************************************************************************************/
void gpt_blinker_blue_callback(timer_callback_args_t *p_args)
{
    /*o_args parameter not used*/
    UNUSED_PARAM(p_args);

    static bool_t s_blink_on = true;
