...
void dhcp_client_thread_entry(void)
{
    ...
    app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR, sizeof("Network Initialization completed successfully."),\
    ...
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("run_dhcp_client_session failed."),\
                           "run_dhcp_client_session failed.");
    ...
    app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR, sizeof("DHCPV4 Client EP completed."),\
                       "DHCPV4 Client EP completed.");
...
static void packet_pool_init0(void)
{
    ...
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_packet_pool_create failed."),\
                           "nx_packet_pool_create failed.");
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_TRAP, sizeof(UINT *), &status);
    }
}

/*******************************************************************************************************************//**
 * @brief     Create the IP instance and enable ARP, UDP, ICMP.
 * @param[IN] None
 * @retval    None
 **********************************************************************************************************************/
static void ip_init0(void)
{
    ...
    if (IP_ADDRESS(0, 0, 0, 0) != G_IP0_GATEWAY_ADDRESS)
    {
        ...
        if (NX_SUCCESS != status)
        {
            app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_ip_gateway_address_set failed."),\
    ...
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_ip_status_check failed."),\
...
static void dhcp_client_init0(void)
{
    ...
    UINT status = nx_dhcp_create(&g_dhcp_client0, &g_ip0, "g_dhcp_client0");
    if (NX_SUCCESS != status)
    {
    ...
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_packet_pool_set failed."),\
...
static UINT run_dhcp_client_session(NX_DHCP *client_ptr, NX_IP *ip_ptr)
{
    ...
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("tx_event_flags_create failed."),\
                           "tx_event_flags_create failed.");
        return status;
    }

    app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR, sizeof("Checking Ethernet Link..."),\
    ...
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_state_change_notify failed."),\
    ...
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR, sizeof("DHCP client is assigned an IP address."),\
                           "DHCP client is assigned an IP address.");

        /* Obtain the client IP address from this NetX service */
        status = nx_ip_address_get(ip_ptr, &client_address, &network_mask);
        if (NX_SUCCESS != status)
        {
            app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_ip_address_get failed."),\
        ...
        if (NX_SUCCESS != status)
        {
            app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_server_address_get failed."),\
        ...
        status = nx_icmp_ping(ip_ptr, server_address, "Hello World", sizeof("Hello World"),\
                              &my_packet, NX_WAIT_FOREVER);
        if (NX_SUCCESS != status)
        {
            app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_icmp_ping failed."), "nx_icmp_ping failed.");
            return status;
        }
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR, sizeof("Successfully Pinged DHCP Server."),\
                           "Successfully Pinged DHCP Server.");

        /* Release the echo response packet once done */
    ...
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_client_get_record failed."),\
                           "nx_dhcp_client_get_record failed.");
        return status;
    }
    /* Print lease time */
    ...
    /* Prepare the DHCP client to restart. We can still send and receive packets except broadcast packets,
     * but with a source IP address of zero, is not very useful except for DHCP */
    status = nx_dhcp_reinitialize(client_ptr);
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_reinitialize failed."),\
                           "nx_dhcp_reinitialize failed.");
        return status;
    }
    app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR, sizeof("DHCP Client is reinitializing..."),\
    ...
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_start failed."),\
    ...
    /* Poll the IP instance directly for a valid IP address */
    do
    {
        ...
        if (status)
        {
            /* Sleep for one IP periodic tick */
    ...
    /* Verify if a valid IP address is assigned */
    if (status == NX_SUCCESS)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR,\
                           sizeof("DHCP client is assigned IP address lease for second time."),\
                           "DHCP client is assigned IP address lease for second time.");
        ...
        if (NX_SUCCESS == status)
        {
            ...
            for (uint8_t cnt = RESET_VALUE; cnt < (dns_buffer_size/IP_V4_SIZE); cnt++, dns_server_ptr++)
            {
                ...
                status = nx_icmp_ping(ip_ptr, *dns_server_ptr, "Hello", sizeof("Hello"),\
                                      &my_packet, (3* NX_IP_PERIODIC_RATE));

                /* No valid ICMP packet received (No packet to release) */
                if (NX_SUCCESS != status)
                {
                    app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_icmp_ping failed."),\
                                       "nx_icmp_ping failed.");
                    return status;
                }
                else
    ...
    /* Query the DHCP Client for the DNS Gateway address */
    ...
    while (status == NX_DHCP_DEST_TO_SMALL)
    {
        ...
        if (gateway_buffer_size < MAX_GATEWAYS * IP_V4_SIZE)
        {
            ...
            status = nx_dhcp_user_option_retrieve(client_ptr, NX_DHCP_OPTION_GATEWAYS,\
                                                  gateway_buffer, &gateway_buffer_size);
        }
        else
        {
            break;
    ...
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_user_option_retrieve failed."),\
                           "nx_dhcp_user_option_retrieve failed.");
        return status;
    }
    else
    {
        ...
        for (uint8_t cnt = RESET_VALUE; cnt < (gateway_buffer_size/IP_V4_SIZE); cnt++, dns_server_ptr++)
        {
            ...
            status = nx_icmp_ping(ip_ptr, *gateway_ptr, "Hello Gateway", sizeof("Hello Gateway"),\
                                  &my_packet, 3* NX_IP_PERIODIC_RATE);
    ...
    /* DHCP client finished. Release the IP address back to the server
     * This application should not send or receive packets with this IP address now */
    status = nx_dhcp_release(client_ptr);
    if (NX_SUCCESS != status)
    {
        app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_ERR_STR, sizeof("nx_dhcp_release failed."),\
                           "nx_dhcp_release failed.");
        return status;
    }

    app_rtt_print_data(RTT_OUTPUT_MESSAGE_APP_INFO_STR, sizeof("Released IP address back to Server."),\
 ...
 *              In this callback, we only check if the DHCP Client has changed to the bound state
 *              (has a valid IP address) and we set a event flag for the application to check.
...
static VOID dhcpv4_client_notify_callback(NX_DHCP *client_ptr, UCHAR state)
{
    ...
    /* Verify transition to bound state (Have a valid IP address) */
