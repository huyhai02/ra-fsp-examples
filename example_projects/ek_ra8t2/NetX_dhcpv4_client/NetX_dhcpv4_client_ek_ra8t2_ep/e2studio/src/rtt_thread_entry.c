 ...
 *            and displays messages to RTT Viewer. Also releases print message allocated block.
 * @param[IN] None
 * @retval    None
 **********************************************************************************************************************/
static void process_rtt_op_msg(VOID)
{
    ...
    do
    {
        ...
        {
            /* Print the current time */
            snprintf(&timestamp_info[0], sizeof(timestamp_info), "Timestamp (Ticks) = 0x%08x:\r\n",\
            ...
            switch (rtt_op_data->msg_id)
            {
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_CLIENT_IP:
                {
                    ...
                    sprintf(ipv4_address, "%d.%d.%d.%d", client_ip.IP_t.ip_a, client_ip.IP_t.ip_b,\
                            client_ip.IP_t.ip_c, client_ip.IP_t.ip_d);
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_SERVER_IP:
                {
                    ...
                    sprintf(ipv4_address, "%d.%d.%d.%d", server_ip.IP_t.ip_a, server_ip.IP_t.ip_b,\
                            server_ip.IP_t.ip_c, server_ip.IP_t.ip_d);
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_DNS_SERVER_IP:
                {
                    ...
                    sprintf(ipv4_address, "%d.%d.%d.%d", dns_server_ip.IP_t.ip_a, dns_server_ip.IP_t.ip_b,\
                            dns_server_ip.IP_t.ip_c, dns_server_ip.IP_t.ip_d);
                ...
                case RTT_OUTPUT_MESSAGE_APP_PRINT_GATEWAY:
                {
                    ...
                    sprintf(ipv4_address, "%d.%d.%d.%d", dns_gateway.IP_t.ip_a, dns_gateway.IP_t.ip_b,\
                            dns_gateway.IP_t.ip_c, dns_gateway.IP_t.ip_d);
...
UINT app_rtt_print_data(event_id_t id, uint32_t size, void * const p_data)
{
    ...
    /* Allocates memory for RTT Viewer display message data structure */
