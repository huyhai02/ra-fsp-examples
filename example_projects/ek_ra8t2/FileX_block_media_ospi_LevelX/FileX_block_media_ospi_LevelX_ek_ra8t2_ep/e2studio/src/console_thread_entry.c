...
void console_thread_entry(void)
{
    ...
    status = tx_event_flags_get(&g_request_event, REQUEST_START_CONSOLE, TX_OR_CLEAR, &actual_event, TX_WAIT_FOREVER);
    ...
    while (true)
    {
        ...
        if (RESET_VALUE != (request & FILE_SYSTEM_REQUEST_MASK))
        {
            ...
            status = tx_event_flags_get(&g_request_event, REQUEST_COMPLETED, TX_OR_CLEAR, &actual_event,
                                        TX_WAIT_FOREVER);
            if (TX_SUCCESS != status)
            {
                PRINT_ERR_STR("tx_event_flags_get for REQUEST_START_CONSOLE failed\r\n");
