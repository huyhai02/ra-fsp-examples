...
fsp_err_t sector_write(void)
{
    ...
    {
        .sector_map_entry = NULL,
        .sector_address = NULL,
        .logical_sector = RESET_VALUE
    };
    _Bool is_empty = true;
    ...
    if (is_empty)
    {
        ...
        err = RM_LEVELX_NOR_SPI_Write(&g_rm_levelx_nor_spi_ctrl, sector.sector_address, (ULONG *)sector_write_data,
                                      SECTOR_WIRTE_SIZE / BYTES_PER_WORD);
...
fsp_err_t sector_read(void)
{
    ...
    if (!is_empty)
    {
        ...
        err = RM_LEVELX_NOR_SPI_Read(&g_rm_levelx_nor_spi_ctrl, sector.sector_address, (ULONG *)read_data,
                                     SECTOR_READ_SIZE / BYTES_PER_WORD);
...
fsp_err_t sector_erase(void)
{
    ...
    if (!is_empty)
    {
        ...
        block = ((ULONG)sector.sector_address - FILEX_START_ADDRESS) / OSPI_B_SECTOR_4K_SIZE;
#endif
    ...
    do
    {
        ...
        *select_sector = (uint16_t)(atoi(recv_buf));
        ...
        if (*select_sector >= LEVELX_SECTOR_RANGE_START && *select_sector <= LEVELX_SECTOR_RANGE_END)
        {
            ...
            *select_sector_address = (ULONG *)(LEVELX_START_ADDRESS + ((*select_sector - LEVELX_SECTOR_RANGE_START) *
            ...
            *select_sector_address = (ULONG *)(LEVELX_START_ADDRESS + ((*select_sector - LEVELX_SECTOR_RANGE_START) *
            ...
            /* Check if sector is empty */
            for (uint32_t n = 0; n < OSPI_B_SECTOR_4K_SIZE; n++)
            {
                UCHAR *byte_address = (UCHAR *)(*select_sector_address) + n;
...
UINT sector_info_get(void)
{
    ...
    lx_err = _lx_nor_flash_logical_sector_find(&g_lx_nor0, sector.logical_sector ,LX_TRUE, &sector.sector_map_entry,
