...
uint32_t term_framework_init(void)
{
    ...
    uint32_t status = tx_byte_pool_create(&g_term_byte_pool, (CHAR*) "Term Byte Pool", (VOID *)g_term_byte_pool_memory,
                                          TERM_BYTE_POOL_SIZE);
    ...
    status = tx_queue_create(&g_term_output_queue, (CHAR*) "Term Output Queue", 1, p_mem, TERM_OUTPUT_QUEUE_SIZE);
    if (TX_SUCCESS != status)
    {
        return status;
    }
    ...
    status = tx_queue_create(&g_term_input_queue, (CHAR*) "Term Input Queue", 1, p_mem, TERM_INPUT_QUEUE_SIZE);
    if (TX_SUCCESS != status)
    {
        return status;
    }
