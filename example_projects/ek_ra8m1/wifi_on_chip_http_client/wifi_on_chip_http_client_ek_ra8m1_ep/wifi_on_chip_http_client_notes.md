...
|configuration.xml > HTTP Client Thread > HTTP Client on DA16XXX (rm_http_onchip_da16xxx) > Wifi DA16XXX Framework Driver (rm_wifi_da16xxx) > DA16XXX transport on UART (rm_at_transport_da16xxx_uart) > Properties > Settings > Property > Module Reset Port | 06 | 08 |Setup reset port for the MCU.|
|configuration.xml > HTTP Client Thread > HTTP Client on DA16XXX (rm_http_onchip_da16xxx) > Wifi DA16XXX Framework Driver (rm_wifi_da16xxx) > DA16XXX transport on UART (rm_at_transport_da16xxx_uart) > Properties > Settings > Property > Module Reset Pin | 03 | 09 |Setup reset pin for the MCU.|
|configuration.xml > g_uart0 UART (r_sci_b_uart) > module > general > channel | 0 | 2 | PMOD2 is available on UART channel 2. |
|configuration.xml > g_uart0 UART (r_sci_b_uart) > Pins > TXD2 | unavailable | PA03 | Select TX pin. |
|configuration.xml > g_uart0 UART (r_sci_b_uart) > Pins > RXD2 | unavailable | PA02 | Select RX pin. |
