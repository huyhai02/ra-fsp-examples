...
1. Project Overview:
	...
	The two example projects ssi_master_ek_ra8m1_ep and ssi_slave_ek_ra8m1_ep are provided for the the SSI Slave EP, one intended for Master and other for the Slave.
...
4. To run this example project, user needs to compile and download the example project to the evaluation kit and then follow below instructions to observe the operation of the system.
   ...
   Supported Boards: EK-RA4M2, EK-RA4M3, EK-RA6M1, EK-RA6M2, EK-RA6M3, EK-RA6M4, EK-RA6M5, EK-RA4E2, EK-RA6E2, EK-RA8M1.
...
5. The user cannot change the configuration from Master to Slave at run time.
	...
	1) The example project ssi_master_ek_ra8m1_ep has Master configuration settings and runs in master mode and the example project ssi_slave_ek_ra8m1_ep has Slave configuration settings and runs in slave mode.
...
6. Hardware settings for the project:
	...
	9) P102 SSIBCK0(Master)-----> P102 SSIBCK0(Slave)
	   ...
	   EK-RA8M1                   EK-RA8M1
	9) P112 SSIBLK0(Master) -----> P112 SSIBLK0(Slave) 
	   P113 SSILRCK0(Master)-----> P113 SSILRCK0(Slave) 
	   P405 SSITXD(Master)  -----> P114 SSIRXD(Slave) 
	   P114 SSIRXD(Master)  -----> P405 SSITXD(Slave) 
	   GND                  -----> GND  
