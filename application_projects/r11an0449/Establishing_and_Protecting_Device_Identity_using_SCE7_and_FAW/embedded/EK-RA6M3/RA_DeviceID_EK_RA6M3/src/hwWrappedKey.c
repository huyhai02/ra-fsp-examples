...
bool hwSignData(uint8_t * hash_buffer, uint8_t sig_buffer[], psa_key_handle_t *key_handle)
{
    ...
    {
        APP_PRINT("R_SCE_HRK_ECC_256PrivateKeySign failed: %x \n\r", (unsigned int)status);
        status = false;
    }
    else
    {
               ...
               if(FSP_SUCCESS == status)
               {
                   ...
                   if (FSP_SUCCESS == status)
                   {
                       ...
                       if(PSA_SUCCESS == psa_status)
                       {
                           ...
                           {
                               APP_PRINT("ECC key generation successful, public key export failed:%x \n\r", (unsigned int)psa_status);
                               ret = false;
                           }
                       }
                       else
                       {
                           APP_PRINT("ECC SECP256R1 key generation failed: %x \n\r", (unsigned int)psa_status);
                           ret = false;
                       }
                   }
                   else
                   {
                       APP_PRINT("Little file system mounting failed: %x \n\r", (unsigned int)status);
                       ret = false;
                   }
               }
               else
               {
                   APP_PRINT("Little file system formatting failed: %x \n\r", (unsigned int)status);
                   ret = false;
               }
           }
           else
           {
               APP_PRINT("Little file system open failed: %x \n\r", (unsigned int)status);
               ret = false;
           }
        }
        else
        {
            APP_PRINT("PSA Crypto init failed:%x \n\r", (unsigned int)psa_status);
            ret = false;
        }
    }
    else
    {
        APP_PRINT("platform set up failed:%x \n\r", (unsigned int)status);
        APP_ERR_TRAP(status);
    }
