...
/**
 * @brief Wait for a touch event on the GT911 touch controller.
 *
 * This function waits for a touch event on the GT911 touch controller by synchronizing with the
 * TOUCH_GT911_INT event. The function blocks until a touch event occurs or an error occurs.
 *
 * @param[in] p_ctrl Pointer to the GT911 touch controller control block.
 *
 * @return fsp_err_t::FSP_SUCCESS on successful synchronization with the touch event, fsp_err_t::FSP_ERR_* on error.
 *
 * @note This function assumes that it is used in an environment where an RTOS is configured.
 *       It utilizes the synchronization event TOUCH_GT911_INT for blocking until a touch event occurs.
 *
 * @warning The behavior of this function depends on the proper configuration of the synchronization
 *          objects and the underlying RTOS. Make sure to configure the RTOS properly to match
 *          the intended behavior of the function.
 */
fsp_err_t R_TOUCH_GT911_WaitForTouch(touch_gt911_ctrl_t * const );
