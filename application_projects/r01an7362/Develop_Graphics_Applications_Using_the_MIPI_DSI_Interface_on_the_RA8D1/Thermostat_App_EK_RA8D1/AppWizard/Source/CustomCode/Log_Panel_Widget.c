...
static fsp_err_t RotaryInit(WM_MESSAGE * pMsg)
{
    ...
    if(hWin_temp)
    {
      APPW_SetVarData(ID_VAR_TEXT_COLOR_CHANGE, (I32)gDataApp.text_color);
...
{
     int x0, y0, xSize, ySize;      // Logging Dialog widget specs
     x0 = 18;
     y0 = 685;
     xSize = 445;
     ySize = 150;
...
/*******************************************************************************************************************//**
 * @brief       Custom code for ID_SCREEN_LOG__WM_NOTIFICATION_VALUE_CHANGED_LCD_BACKLIGHT in ID_SCREEN_LOG_Slots.c
...
void cusID_SCREEN_LOG__WM_NOTIFICATION_VALUE_CHANGED(WM_MESSAGE * pMsg) {
  ...
  {
    int gui_err = 0;
    gDataApp.text_color = (uint32_t)APPW_GetVarData(ID_VAR_TEXT_COLOR_CHANGE, &gui_err);
    if(gui_err)
    {
      APP_ERR_TRAP(gui_err);
    }
