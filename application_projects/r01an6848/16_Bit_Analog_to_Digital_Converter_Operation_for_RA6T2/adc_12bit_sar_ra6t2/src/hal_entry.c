...
static volatile int32_t  adc12_vals[NUM_SAMPLES] = {0};
...
void hal_entry (void)
{
    ...
    err = R_ADC_B_Open(&g_adc_12bit_ctrl, &g_adc_12bit_cfg);
    if (FSP_SUCCESS != err)
    {
    ...
    err = R_ADC_B_ScanCfg(&g_adc_12bit_ctrl, &g_adc_12bit_scan_cfg);
    if (FSP_SUCCESS != err)
    {
    ...
    while (1)
    {
        ...
        else
        {
            b_no_input = true;
            APP_PRINT(
                "If conditions changed since last conversion and conditions meet the requirements for re-calibration, enter '3' on keyboard.\n If not, to skip re-calibration enter '4'.\r\n ");
            while (b_no_input)
            {
                if (APP_CHECK_DATA)
                {
        ...
        err = R_ADC_B_ScanGroupStart(&g_adc_12bit_ctrl, ADC_GROUP_MASK_0);
        if (FSP_SUCCESS != err)
        {
...
void calibration_procedure (void)
{
    ...
    err = R_ADC_B_Calibrate(&g_adc_12bit_ctrl, NULL);
    if (FSP_SUCCESS != err)
    {
           ...
           (FSP_SUCCESS == err))
    {
        ...
        err = R_ADC_B_StatusGet(&g_adc_12bit_ctrl, &status);
    }
...
void cleanup_procedure (void)
{
    ...
    err = R_ADC_B_ScanStop(&g_adc_12bit_ctrl);
    if (FSP_SUCCESS != err)
    {
...
void cb_scan_end (adc_callback_args_t * p_args)
{
    ...
    else
    {
        ...
        err = R_ADC_B_Read(&g_adc_12bit_ctrl, ADC_CHANNEL_0, &channel_0_conversion_result);
        if (FSP_SUCCESS != err)
        {
        ...
        adc12_vals[sample_iter] = (int32_t) channel_0_conversion_result;
        ++sample_iter;
    }
}
