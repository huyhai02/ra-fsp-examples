...
static fsp_err_t qspi_init(void)
{
    fsp_err_t ret = FSP_SUCCESS;

    ret = R_QSPI_Open(&g_qspi0_ctrl, &g_qspi0_cfg);
    if (ret != FSP_SUCCESS)
    {
        return FSP_ERR_NOT_OPEN;
    }

    ret = R_QSPI_DirectWrite(&g_qspi0_ctrl, &(g_qspi0_cfg.write_enable_command), 1, false);
    if (ret != FSP_SUCCESS)
    {
        return FSP_ERR_WRITE_FAILED;
    }

    ret = R_QSPI_DirectWrite(&g_qspi0_ctrl, data_sreg, SREG_SIZE, false);
    if (ret != FSP_SUCCESS)
    {
        return FSP_ERR_WRITE_FAILED;
    }
