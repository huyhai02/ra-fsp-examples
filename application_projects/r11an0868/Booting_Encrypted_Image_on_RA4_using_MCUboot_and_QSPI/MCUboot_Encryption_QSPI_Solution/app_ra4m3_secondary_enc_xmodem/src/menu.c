...
#if 0
                    err = R_FLASH_HP_Erase(&g_flash0_ctrl,
                                           (uint32_t) SECONDARY_IMAGE_START_ADDRESS,
                                           SECONDARY_IMAGE_NUM_BLOCKS);
                    if (FSP_SUCCESS == err)
                    {
                        err = R_FLASH_HP_BlankCheck(&g_flash0_ctrl,
                                                    SECONDARY_IMAGE_START_ADDRESS,
                                                    SECONDARY_IMAGE_END_ADDRESS - SECONDARY_IMAGE_START_ADDRESS + 1,
                                                    &blank_check_result);
                        if (FSP_SUCCESS == err)
                        {
