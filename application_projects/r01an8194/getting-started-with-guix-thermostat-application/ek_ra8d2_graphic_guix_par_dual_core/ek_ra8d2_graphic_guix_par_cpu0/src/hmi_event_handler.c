...
static GX_WIDGET* settings_content_ptr_from_id(USHORT id);
static GX_WIDGET* settings_menu_ptr_from_id(USHORT id);


// Macros: content pointers
#define PTR_SETTINGS_CONTENT_DISPLAY   ((GX_WIDGET *)&Settings.Settings_Display)
#define PTR_SETTINGS_CONTENT_TIME      ((GX_WIDGET *)&Settings.Settings_SetTime)
#define PTR_SETTINGS_CONTENT_DATE      ((GX_WIDGET *)&Settings.Settings_SetDate)
#define PTR_SETTINGS_CONTENT_THERMO    ((GX_WIDGET *)&Settings.Settings_Thermostat)
#define PTR_SETTINGS_CONTENT_NONE      ((GX_WIDGET *)&Settings.Settings_BlankContent)

// Macros: menu item pointers (highlight)
#define PTR_SETTINGS_MENU_DISPLAY      ((GX_WIDGET *)&Settings.Settings_MenuItemDisplay)
#define PTR_SETTINGS_MENU_TIME         ((GX_WIDGET *)&Settings.Settings_MenuItemTime)
#define PTR_SETTINGS_MENU_DATE         ((GX_WIDGET *)&Settings.Settings_MenuItemDate)
#define PTR_SETTINGS_MENU_THERMO       ((GX_WIDGET *)&Settings.Settings_MenuItemThermostat)
#define PTR_SETTINGS_MENU_NONE         ((GX_WIDGET *)&Settings.Settings_MenuItemNone)
...
UINT thermostat_screen_event(GX_WINDOW * p_window, GX_EVENT *event_ptr)
{
    ...
    switch (event_ptr->gx_event_type)
    {
        ...
        case GX_SIGNAL(ID_HELP_BUTTON, GX_EVENT_CLICKED):
            ...
            update_time ((GX_WIDGET *) p_help_screen, &g_gui_state);
            update_date((GX_WIDGET *) p_help_screen, &g_gui_state);
            break;
...
UINT settings_screen_event(GX_WINDOW *widget, GX_EVENT *event_ptr)
{
    ...
    switch (event_ptr->gx_event_type)
    {
        ...
        case GXEVENT_MSG_TIME_UPDATE:
            ...
            switch(cur_settings_screen) { // @suppress("10.3a Default statement")
        ...
        case GX_SIGNAL(ID_HOUR_MODE_TOGGLE, GX_EVENT_CLICKED):
            ...
            if (SYSTEM_HOUR_MODE_12 == g_gui_state.hour_mode)
            {
                update_text_id((GX_WIDGET *) widget, ID_TIME_FORMAT_TEXT, GX_STRING_ID_FORMAT_12);
            }
            else
            {
                update_text_id((GX_WIDGET *) widget, ID_TIME_FORMAT_TEXT, GX_STRING_ID_FORMAT_24);
...
static void send_message(system_msg_t * p_message)
{
    ...
    /* Use timeout instead of TX_NO_WAIT to prevent system trap on queue full */
    tx_err = tx_queue_send((TX_QUEUE*)&system_msg_queue, (VOID*)p_message, 10);
    /* Trap here if err*/
...
static UINT settings_item_show(GX_WINDOW *widget, USHORT new_content, USHORT old_content) {
    ...
    GX_PARAMETER_NOT_USED(widget);
    UINT gx_err = GX_SUCCESS;

    GX_BOOL   return_moved = GX_FALSE;
    GX_WIDGET *pOldContent = settings_content_ptr_from_id(old_content);
    GX_WIDGET *pNewContent = settings_content_ptr_from_id(new_content);

    /* Fast paths: nothing changed */
    if (new_content == old_content) {
        return GX_SUCCESS;
    }
    if (pNewContent && pOldContent && (pNewContent == pOldContent)) {
        return GX_SUCCESS;
    }

    /* Hide old first to avoid overdraw/flicker */
    if (pOldContent && (pOldContent->gx_widget_status & GX_STATUS_VISIBLE)) {
        gx_err = gx_widget_hide(pOldContent);
        if (gx_err != GX_SUCCESS && gx_err != GX_NO_CHANGE) {
            APP_ERR_TRAP(FSP_ERR_ASSERTION);
        }
    }

    /* Show new (BlankContent for NONE as mapped) */
    if (pNewContent && !(pNewContent->gx_widget_status & GX_STATUS_VISIBLE)) {
        gx_err = gx_widget_show(pNewContent);
        if (gx_err != GX_SUCCESS && gx_err != GX_NO_CHANGE) {
            APP_ERR_TRAP(FSP_ERR_ASSERTION);
        }
    }

    /* Ensure new content is on top within its parent */
    if (pNewContent) {
...
static void settings_menu_highlight(GX_WINDOW *widget, USHORT new_highlight) {
    ...
    /* No work if selection doesn't change */
    if (new_highlight == cur_menu_highlight) {
        return;
    }

    /* Remove current highlight if any */
    GX_WIDGET *pCur = settings_menu_ptr_from_id(cur_menu_highlight);
    if (pCur && !(pCur->gx_widget_style & GX_STYLE_TRANSPARENT)) {
        (void) gx_widget_style_add(pCur, GX_STYLE_TRANSPARENT);
    }

    /* If new highlight is NONE, just update state and return */
    if (new_highlight == ID_SETTINGS_MENUITEM_NONE) {
        cur_menu_highlight = ID_SETTINGS_MENUITEM_NONE;
        return;
    }

    /* Apply new highlight */
    GX_WIDGET *pNew = settings_menu_ptr_from_id(new_highlight);
    if (pNew && (pNew->gx_widget_style & GX_STYLE_TRANSPARENT)) {
        (void) gx_widget_style_remove(pNew, GX_STYLE_TRANSPARENT);
    }
...
/*******************************************************************************************************************//**
 * @brief   Returns a pointer to the settings content widget based on its ID.
 *
 * @param[in]   id  The resource ID of the settings content widget
 *
 * @retval      Pointer to the GX_WIDGET or GX_NULL if not found
 **********************************************************************************************************************/
static GX_WIDGET * settings_content_ptr_from_id(USHORT id)
{
    switch (id) {
        case ID_SETTINGS_CONTENT_DISPLAY: return PTR_SETTINGS_CONTENT_DISPLAY;
        case ID_SETTINGS_CONTENT_TIME:    return PTR_SETTINGS_CONTENT_TIME;
        case ID_SETTINGS_CONTENT_DATE:    return PTR_SETTINGS_CONTENT_DATE;
        case ID_SETTINGS_CONTENT_THERMO:  return PTR_SETTINGS_CONTENT_THERMO;
        case ID_SETTINGS_CONTENT_NONE:    return PTR_SETTINGS_CONTENT_NONE;
        default:                          return GX_NULL;
    }
}

/*******************************************************************************************************************//**
 * @brief   Returns a pointer to the settings menu item widget based on its ID.
 *
 * @param[in]   id  The resource ID of the settings menu item widget
 *
 * @retval      Pointer to the GX_WIDGET or GX_NULL if not found
 **********************************************************************************************************************/
static GX_WIDGET* settings_menu_ptr_from_id(USHORT id)
{
    switch (id) {
        case ID_SETTINGS_MENUITEM_DISPLAY:    return PTR_SETTINGS_MENU_DISPLAY;
        case ID_SETTINGS_MENUITEM_TIME:       return PTR_SETTINGS_MENU_TIME;
        case ID_SETTINGS_MENUITEM_DATE:       return PTR_SETTINGS_MENU_DATE;
        case ID_SETTINGS_MENUITEM_THERMOSTAT: return PTR_SETTINGS_MENU_THERMO;
        case ID_SETTINGS_MENUITEM_NONE:       return PTR_SETTINGS_MENU_NONE; // NULL
        default:                              return GX_NULL;
    }
}
/** @} (end group events) */
